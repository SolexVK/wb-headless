...
3. **Каркас бота** (grammY): меню из реестра, FSM, персист состояния, очередь+воркеры.
   ...
   - 3b ✅ **Очередь + реальное выполнение.** `bot/core/queue.js` (глобальный лимит
     конкурентности + дорожки-семафоры под лимиты источников), `bot/core/executor.js`
     (spawn `npm run <script>`, чтение JSON-контракта, ошибки без исключений). На
     «Запустить» — `createRun` → очередь → CLI → `finishRun` → выдача в чат владельца:
     текстовая сводка ТОП-конкурентов + JSON-контракт файлом. Проверено на реальных данных
     MPStats (99 в выдаче → 10 конкурентов). Прогон пишется в `skill_runs`.
   - 3c ✅ **Пикер конкурентов + выбор формата + выдача.** `bot/render/` (renderReport →
     HTML через `formatHtml`, PDF через `htmlToPdf`, XLSX через `xlsx`). После выполнения —
     клавиатура действий: форматы (📄HTML/📑PDF/📊XLSX, рендер лениво + кэш в
     `report_files`/`skill_artifacts`, повторная выдача из кэша) и 🎯 пикер конкурентов
     **фото-пикер**: топ-N кандидатов — каждый фото-сообщением с тумблером ⬜/✅ под ним
     (webp Telegram по URL не берёт → шлём БАЙТАМИ), выбор 2–4 → `_selected` в прогоне для
     будущей передачи в «Сравнение карточек»). Всё привязано к `runId` в `skill_runs`
     (переживает рестарт).
     Рендер всех форматов и плумбинг проверены на фикстуре (PDF валиден, кэш байт-в-байт).
...
## 12. Чистота выдачи: фильтр по характеристикам + рисунок (уровни)

Проблема: минус-ключи фильтруют по НАЗВАНИЮ, но отличающий признак (крой, вырез,
рукав, застёжка, рисунок) часто есть только на фото/в характеристиках карточки →
«грязь». Решение — многоуровневое:

- **Уровень 0** — фильтр по названию (был). Дырявый.
- **Уровень 1 ✅** — фильтр по **характеристикам карточки** (`card.json` с basket-CDN:
  Покрой/Вырез/Рукав/Застёжка/Декор + описание). `lib/wbCard.js` (`fetchCardRaw`,
  `enrichRows`, `classifyPattern`/`patternVerdict`). Минус-ключи/группы матчатся по
  `название + характеристики + описание`. Бесплатно, детерминированно, файловый кэш по
  nmId (только успехи; промахи повторяются). Проверено: карточка с «оверсайз» лишь в
  характеристиках теперь отсекается по `--exclude оверс`.
- **Признаки-фасеты ✅** — обобщение: справочник `lib/wbFacets.js` (Тип ткани, Покрой,
  Воротник, Застёжка, Рукав, Карманы, Рисунок, Пол). Пользователь выбирает значение признака
  (плюс-ключ) → **остальные значения того же признака автоматически = минус-ключи**. Вердикт
  на признак keep/drop/uncertain; карточка проходит, если ни один фасет не даёт drop; 🟡 —
  если по признаку сигнала нет. CLI: `--facet cut=оверсайз` (повторяемый). В боте поля
  генерируются из справочника (choice, advanced) — настраиваются ТОЛЬКО релевантные через
  «⚙ Настроить». На реальной выдаче «в полоску» (оверсайз+полоска): 15 конкурентов, 🟡
  помечены.
- **Уровень 2 ⏳** — **vision** (Haiku) по миниатюре ТОЛЬКО для 🟡-бакета (рисунок не
  тегнут), кэш по nmId навсегда, мягкая метка. Для «только однотонная» без него не обойтись.
- **Уровень 3** — человек добивает 🟡 глазами в пикере (фото уже показываем).
