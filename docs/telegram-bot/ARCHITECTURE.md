...
- **Рисунок ткани** — отдельная ось (`--pattern однотон/полоска/клетка/…`). Классификатор
  по названию+характеристикам+описанию: конфликт → drop, совпадение → keep, **нет сигнала
  → 🟡 uncertain** (не выбрасываем молча — помечаем в сводке/пикере). На реальной выдаче
  «в полоску»: 18/19 определены точно, 1 🟡.
