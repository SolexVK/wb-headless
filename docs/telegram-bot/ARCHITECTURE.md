...
1. **Каркас данных.** Миграция БД v2 поверх ветки с БД (`source_cache`, `report_files`,
   `card_variants`, `skill_runs`, `skill_artifacts`, `users`, `user_settings`) + хелперы
   чтения/записи и дедупа.
...
- Точная библиотека бота (предлагаю **grammY**) — подтвердить.
