...
Снапшоты в SCP (лимит — два): `base-debian13-node-wbheadless` (03.09.2026, до Caddy и
домена) и **`2026-09-05-planner-demo-backups`** (05.09.2026) — актуальная точка возврата
с planner, демо-стендом и бэкапами. При создании снапшота сервер перезагружается;
проверено, что все службы поднимаются сами.

Грабля бэкапов: первые архивы выходили по 9 КБ — временный каталог создавался root с
правами 700, и владелец базы не мог записать туда снимок, а ошибка гасилась. Теперь
архив проверяется по содержимому (есть ли `planner.db` и не мал ли он), иначе запуск
падает. Урок общий: проверять бэкап по содержимому, а не по факту создания файла.
