...
Вход — через **аккаунты** (форма входа, не Basic Auth). `CALC_USER`/`CALC_PASS` при первом
запуске создают **аккаунт админа** и печатают его код восстановления в лог.
```bash
cd docs/china-fbs/calculator
CALC_USER="ваш_логин" CALC_PASS="надёжный_пароль" node serve.cjs
```
Увидите: `Калькулятор с аккаунтами: http://127.0.0.1:8787` и строку `Создан админ … Код
восстановления: XXXX-XXXX-XXXX` — сохраните код. Проверка: неавторизованный запрос
редиректит на вход:
```bash
curl -s -o /dev/null -w '%{http_code} %{redirect_url}\n' http://127.0.0.1:8787/   # 302 …/login
```
Вход/регистрация/восстановление — через браузер: `/login`, `/register`, `/forgot`; одобрение
новых пользователей — на `/admin` (под админом).
