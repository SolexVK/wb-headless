...
|---|---|---|
| 443 | `127.0.0.1:9000` (**Caddy**) → `/`→8477 planner, `/calc`→8899 tandemtrace | обратный прокси |
...
**Порт 443 вынесен за единый обратный прокси Caddy** (2026-08-14). Раньше 443 мультиплексировал
planner `/` и tandemtrace `/calc` через `tailscale funnel --set-path`, а служба `com.tandem.funnel`
(`KeepAlive`) каждые ~10 сек переприменяла `/calc` → гонки за общий funnel-конфиг ломали соседа.
Теперь весь 443 → Caddy (`127.0.0.1:9000`), маршрутизация по путям в `Caddyfile`, служба
`com.tandem.funnel` выключена (`launchctl disable`). Новые сервисы на 443 добавляются правкой
`Caddyfile` + `caddy reload` — Funnel не трогаем. Подробности и правила — `deploy/CADDY.md`.
