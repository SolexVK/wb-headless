...
function computeUnit(v) {
  ...
  const costs = {
    ...
    ff:        ffFirst,                               // первичная обработка ФФ (1× на штуку)
    commission,
    fin,
    logistics: wbLogistics,                          // логистика WB на все заказы (вкл. возвраты)
    ffReturn:  ffReturn * returnsPerSale,            // обработка возвратов ФФ (за каждый возврат)
...
function render() {
  ...
  // Поле тарифа селлера показываем только для «доставка силами селлера»
  ...
  document.getElementById('prices').innerHTML = `
    ...
    <div class="price-note">Витрина — по Курсу ВБ ${fmt(r.wbRate, 2)} ₽/¥ (ЦБ ${fmt(r.cbr, 2)} + надбавка). Прибыль в ₽ — по курсу ЦБ. Доставка Китай→ФФ ${fmt(v.chinaFreightPerKg, 2)} $/кг → ${fmt(r.firstMile, 2)} ¥/шт. ФФ: первичная ${fmt(v.ffFeeRub, 0)} ₽ ≈ ${fmt(r.ffFirst, 2)} ¥ + возвраты ${fmt(v.ffReturnFeeRub, 0)} ₽ × ${fmt(r.returnsPerSale, 2)} ≈ ${fmt(r.ffReturnUnit, 2)} ¥.</div>`;

  const maxCost = Math.max(r.revenue, ...Object.values(r.costs));
  const rows = Object.keys(COST_LABELS).map(k => {
    const val = r.costs[k];
    // Скрываем нерелевантную строку логистики в зависимости от способа доставки
    if (k === 'firstMile' && r.isSeller === false) return '';
