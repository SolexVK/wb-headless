...
# -*- coding: utf-8 -*-
"""
build_seller.py — собирает PDF конкурентного анализа продавца из seller_data.json
(готовит collect_seller.py). Страницы: сводка · разбор целевых карточек ·
конкуренты · логистика складов · потенциал и план распределения стока · ниша ·
дорожная карта.

    python3 build_seller.py [seller_data.json] [--pdf out.pdf] [--html out.html]

Рендер — системным Chromium (--print-to-pdf); путь из $CHROME или автопоиск.
Акценты страниц разведены через CSS-переменные. Названия складов — из
wb_warehouses.json (через reports-generator/build.py).
"""
import json, sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import build as B
nf, money_short, wb_item, wb_brand, a_item, a_brand, wh_label, vars_style = (
    B.nf, B.money_short, B.wb_item, B.wb_brand, B.a_item, B.a_brand, B.wh_label, B.vars_style)
...
def build_all(D):
    M = D['meta']; NI = D['niche']; CM = D['comparable']; CP = D['compare']; WH = D['warehouses']; POT = D['potential']
    ...
    def cover():
        ...
        verdict = f"""<div class="verdict">
          ...
          <div class="vbox" style="--c:#E8663D"><div class="t">Цена vs рынок</div><div class="n">{nf(CP['our_avg_price'])} <small>₽</small></div><p>медиана комплектов {nf(CP['kpb_median_price'])} ₽ — вы <b>не дешёвые</b></p></div>
        ...
        <div class="lead2">Почему у карточек низкие продажи и что усилить. Данные MPStats API за {M['d1']} – {M['d2']} ({M['days']} дней). Ниша — {nf(M['n_sku'])} SKU, сопоставимых комплектов — {nf(CM['n'])}. Дата: {M['generated']}.</div>
        {verdict}
        <div class="tak"><h3>Главные выводы</h3><ul>
          ...
          <li><b>Дело не в цене.</b> Средняя цена <b>{nf(CP['our_avg_price'])} ₽</b> — выше медианы комплектов ({nf(CP['kpb_median_price'])} ₽). Демпинг не обязателен.</li>
    ...
    # ---------- TARGET CARD ----------
    def target_card(sku, pal, anchor, idx):
        ...
        assess = (f"Цена <b>{nf(price)} ₽</b> ({vs_med} к медиане комплектов) — <b>не дешёвая</b>. Продажи <b>{t['sales']} шт/30д</b> "
                  f"({money_short(t['revenue'])} ₽), ранг <b>{nf(t['rank'])}</b> (комплекты <b>{nf(t.get('kpb_rank'))}</b>). "
        ...
        <div class="kpis">
          ...
          {kpi('Цена vs медиана КПБ', f"{nf(price)} <small>₽</small>", f"медиана {nf(CP['kpb_median_price'])} ₽ · {vs_med} · до скидки {nf(t.get('base'))}")}
          {kpi('Выкуп по нише', f"{t.get('buyout','—')}%", 'средняя оценка MPStats по категории')}
          {kpi('Ранг · доля', f"#{nf(t['rank'])}", f"из {nf(M['n_sku'])} · комплекты #{nf(t.get('kpb_rank'))} · доля {t['share']}%", True)}
        ...
        <div class="foot"><span>Источник: MPStats API · период {M['d1']}–{M['d2']}</span><span>Сравнение с сегментом комплектов (КПБ)</span></div>"""
        return page('card-report', pal, inner, anchor)

    # ---------- COMPETITORS ----------
    def competitors_page(anchor):
        rows = ''
        ...
        inner = f"""
        <div class="eyebrow">Конкуренты · сегмент комплектов</div>
        <h1 style="font-size:21px;margin:4px 0 7px">ТОП-конкуренты по выручке в нише</h1>
        <div class="chips"><span class="chip">Сопоставимых комплектов: {nf(CM['n'])} SKU</span><span class="chip">Выручка сегмента {money_short(CM['rev'])} ₽/30д</span></div>
        ...
        <div class="grid2" style="margin-top:12px">
          ...
          <div class="card"><h3>Что говорят цифры</h3>
            <div class="callout" style="margin-bottom:8px">Лидер объёма — комплект ~<b>{nf(lead.get('price'))} ₽</b>, {nf(lead.get('sales'))} продаж/мес, {nf(lead.get('comments'))} отзывов.</div>
        ...
        <div class="foot"><span>Источник: MPStats API · период {M['d1']}–{M['d2']}</span><span>Конкуренты — ТОП по выручке среди комплектов (исключены подушки/одеяла/простыни)</span></div>"""
        return page('niche-report', P_COMP, inner, anchor)

    # ---------- WAREHOUSES ----------
    def warehouse_page(anchor):
        ...
        for hub in WH['central_priority']:
            ...
            covbars += (f'<div class="cmpbar"><span class="lb">{hub}</span><div class="tk"><div class="fl" style="width:{cnt/12*100:.0f}%;background:#C7891B">{cnt}/12</div></div>'
        ...
        <div class="kpis">
          ...
          {kpi('Где вы есть', f"{sw['n_wh']}", 'склада — регион (Рязань, Волгоград…)')}
        ...
        <div class="grid2">
          ...
          <div class="card"><h3>Покрытие центр. хабов <span>сколько из 12 ТОП-конкурентов там есть</span></h3>{covbars}
        ...
        return page('niche-report', P_WH, inner, anchor)

    # ---------- POTENTIAL + DISTRIBUTION ----------
    def potential_page(anchor):
    ...
    def niche_page(anchor):
        ...
        <div class="kpis">
          ...
          {kpi('Медианная цена (комплекты)', f"{nf(CM['price_median'])} <small>₽</small>", f"коридор {nf(CM['p25'])}–{nf(CM['p75'])} ₽ · вся ниша {nf(NI['price_median'])} ₽")}
        ...
        <div class="grid2">
          <div class="card"><h3>Ценовая структура <span>доля выручки по цене</span></h3>{price_bars}
            <div class="callout" style="margin-top:9px">Комплекты живут в коридоре <b>{nf(CM['p25'])}–{nf(CM['p75'])} ₽</b>. Ваши {nf(CP['our_avg_price'])} ₽ — в верхней части, но не премиум.</div></div>
        ...
        <div class="foot"><span>Источник: MPStats API · период {M['d1']}–{M['d2']}</span><span>Сегмент комплектов выделен из подкатегории</span></div>"""
        return page('niche-report', P_NICHE, inner, anchor)

    # ---------- ROADMAP ----------
    def roadmap_page(anchor):
        ...
        return page('niche-report', P_ROAD, inner, anchor)

    pages = [cover()]
    for i, sku in enumerate(D['detailed']):
        pages.append(target_card(sku, P_CARDS[i % len(P_CARDS)], f'p{i+2}', i))
    n = len(D['detailed'])
    pages.append(competitors_page(f'p{n+2}'))
    pages.append(warehouse_page(f'p{n+3}'))
    pages.append(potential_page(f'p{n+4}'))
    pages.append(niche_page(f'p{n+5}'))
    pages.append(roadmap_page(f'p{n+6}'))
    return ("<!doctype html><html><head><meta charset='utf-8'><style>@page{size:A4;margin:0}"
            + B.CSS + EXTRA_CSS + "</style></head><body>" + "\n".join(pages) + "</body></html>")


def main(argv):
    data_path = 'reports-output/seller_data.json'; html_out = None; pdf_out = None
    i = 0
    while i < len(argv):
        if argv[i] == '--html': html_out = argv[i + 1]; i += 2
        elif argv[i] == '--pdf': pdf_out = argv[i + 1]; i += 2
        else: data_path = argv[i]; i += 1
    D = json.load(open(data_path, encoding='utf-8'))
    html = build_all(D)
    html_out = html_out or 'reports-output/seller_report.html'
    open(html_out, 'w', encoding='utf-8').write(html)
    print('HTML →', html_out)
    if pdf_out and B.render_pdf(html_out, pdf_out):
        print('PDF  →', pdf_out)
