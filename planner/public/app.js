  ...
  root.innerHTML = `${globalDistHTML()}
    <div class="panel">
      ...
      <div class="matrix-io" style="margin-top:12px">
        ...
        <label class="btn btn-subtle">⤒ загрузить (.xlsx)<input type="file" accept=".xlsx,.xls,.tsv,.txt,.csv" id="mx-import" hidden></label>
      </div>
    </div>`;
...
async function exportAllSeasonRanksZip(onProgress) {
  ...
  try {
    for (let i = 0; i < list.length; i++) {
      ...
      const baseRows = seasonAgg(p.forecastDaily, 'day', buyout); // ЕЖЕДНЕВНО на весь период (без сжатия)
...
// Авто-подбор цветов по месяцам: для каждого довоза берём месяц его прихода на WB (= старт продаж),
...
function _autoPickApply(articleId, scope) {
  ...
  // Окно продаж довоза = ВСЕ месяцы, которые он закрывает (partiaCoverage): от прихода на WB
  // до прихода следующего довоза; последний — хвост ~2 мес. «Шьём раньше — продаём позже» учтено:
  // считаем от даты прихода (= старт продаж).
  const winCache = {};
  const winMap = (p) => { if (!winCache[p.id]) { const cov = partiaCoverage(articleId, p); winCache[p.id] = new Map(aggColorWindow(cam, cov.baseMM, 0, cov.span).colors.map((x) => [x.name, x.rawShare])); } return winCache[p.id]; };
  ...
  // 1) решение по каждому целевому довозу: выключаем известные рынку цвета с долей < порога (в т.ч. не в сезон = 0%)
...
// Покрытие довоза: месяцы продаж, которые он закрывает (от прихода до прихода следующего довоза;
// последний — хвост ~2 мес). Несколько партий с одним сроком (сплит по цехам) = один довоз.
function partiaCoverage(articleId, p) {
  if (!p || !p.deadline) return { span: 0, baseMM: null, months: [], sellStartISO: '' };
  const allParts = articlePlanPartias(articleId).filter((x) => x.deadline);
  const arrivals = [...new Set(allParts.map((x) => x.deadline))].sort();
  const i = arrivals.indexOf(p.deadline);
  const next = arrivals[i + 1];
  const monthDiff = (f, t) => (+t.slice(0, 4) - +f.slice(0, 4)) * 12 + (+t.slice(5, 7) - +f.slice(5, 7));
  const span = next ? Math.max(1, monthDiff(p.deadline, next)) : MX_COLOR_SPAN;
  const baseMM = p.deadline.slice(5, 7);
  const months = []; for (let k = 0; k < span; k++) months.push(mmShift(baseMM, k));
  return { span, baseMM, months, sellStartISO: p.deadline };
}
function monthsLabel(months) {
  if (!months || !months.length) return '—';
  return months.length > 1 ? `${MX_MON_RU[+months[0]]}–${MX_MON_RU[+months[months.length - 1]]}` : MX_MON_RU[+months[0]];
}
// Тайминг довоза: даты пошива (обратным счётом от прихода на WB по мощности выбранного цеха)
// и зафиксированная дата старта продаж (= приход на WB).
function partiaTiming(a, p) {
  if (!p || !p.deadline) return null;
  const units = matrixSum(p.planMatrix) || 0;
  const ws = p.workshopId ? state.workshops.find((w) => w.id === p.workshopId) : null;
  const cap = ws ? (+ws.capacities.sew || 0) : dailySewCap(a);
  const prod = cap > 0 ? prodDaysCal(units, cap) : 0;
  const transit = transitLeadCal(), buf = riskBufCal();
  const finish = rhythmDate(p.deadline, -transit);                 // готовность пошива (до логистики)
  const sewStart = rhythmDate(p.deadline, -(transit + prod + buf)); // когда садиться шить
...
function colorForPartiaHTML(a, p, parts) {
  ...
  const partiaSel = `<label>Довоз / партия: <select id="mx-partia">${(parts || []).map((x) => `<option value="${x.id}"${p && x.id === p.id ? ' selected' : ''}>${seEsc(partiaLabel(x))}</option>`).join('') || '<option>нет партий</option>'}</select></label>`;
  const cov = partiaCoverage(a.id, p);
  const t = partiaTiming(a, p);
  const monLbl = monthsLabel(cov.months);
  const timingLine = t
    ? `<div class="mx-cm-timing">🧵 <b>Шить ${df(t.sewStart)} — ${df(t.finish)}</b> &nbsp;→&nbsp; 📦 <b>Продажи с ${df(t.sellStart)}</b> <span class="mx-cm-period">${monLbl}</span> <span class="mini">· цех ${seEsc(t.wsName)}, ${t.units.toLocaleString('ru')} шт, ~${t.prod} раб. дн пошива</span></div>`
  ...
  if (!cam) return wrap('<div class="mini">Помесячный анализ цветов недоступен — построй план на «Ранге сезонности».</div>');
  if (cam.stale || !cam.months.length) return wrap(`<div class="mini">Помесячный анализ появится после <b>пересборки плана</b>: «Ранг сезонности» → артикул <b>${seEsc(a.id)}</b> → «Построить план».</div>`);
  if (!cov.months.length) return wrap('<div class="mini">У этого довоза не задан «Срок WB» — не могу определить месяцы продаж для подбора цветов.</div>');
  // ранжирование цветов за ОКНО ПРОДАЖ довоза (все его месяцы)
  const agg = aggColorWindow(cam, cov.baseMM, 0, cov.span);
  ...
  // (джинса↔синий, хаки↔зелёный, шоколад↔коричневый). Показываем ВСЕ цвета артикула; рыночные каноны
  // без моего цвета — «нет в артикуле».
  ...
  const rows = [...artRows, ...extraRows].sort((x, y) => y.share - x.share).slice(0, 40).map((r) => {
    const tier = r.noData ? '' : `tier-${tierOf(r.share)}`;
    ...
    return `<div class="mx-cm-row ${tier} ${r.inArt ? 'in' : 'out'}">
      ...
      <div class="mx-cm-main"><div class="mx-cm-name">${seEsc(r.label)}${note}</div><div class="mx-cm-ul"></div></div>
  ...
  const weakLine = weak.length
    ? `<div class="mx-cm-weak mini">Слабы в продажах «${monLbl}»: <b>${weak.map(seEsc).join(', ')}</b> — кандидаты выключить в этом довозе (чипами ниже).</div>`
    : `<div class="mx-cm-weak mini">Все цвета артикула заметны в продажах «${monLbl}».</div>`;
  return wrap(`
    <div class="mini" style="margin:6px 0 8px">Актуальность цветов в <b>продажах ${monLbl}</b> (окно этого довоза; рыночные аналоги совмещены — джинса↔синий, хаки↔зелёный). Полоса: <b style="color:#22c55e">зелёная ≥5%</b> · <b style="color:#facc15">жёлтая 3–5%</b> · <b style="color:#ef4444">красная &lt;3%</b>. Отмеченные чипами ниже — что реально поедет.</div>
...
async function renderSeasonView(articleId) {
  ...
  bindSeasonCustomColors(rep, p, articleId); // «мои цвета» — добавить/править/убрать
...
// «Мои цвета» — расцветки, которых НЕТ в рыночной выборке, но которые продавец уже продаёт/шьёт.
// Хранятся в article.customColors = [{name, mode:'pct'|'qty', value}] (только валидные активные
// цвета артикула). Кол-во: mode 'qty' → абсолют (шт), 'pct' → доля от лидера (forecast).
function customColorRows(article, forecast, existingNames) {
  const list = (article && Array.isArray(article.customColors)) ? article.customColors : [];
  const active = new Set(activeColors(article));
  const F = Math.max(0, +forecast || 0);
  const seen = new Set(); const out = [];
  for (const cc of list) {
    const name = String((cc && cc.name) || '').trim();
    if (!name || seen.has(name) || (existingNames && existingNames.has(name)) || !active.has(name)) continue;
    seen.add(name);
    const mode = cc.mode === 'qty' ? 'qty' : 'pct';
    const value = Math.max(0, +cc.value || 0);
    const qty = mode === 'qty' ? Math.round(value) : Math.round(F * value / 100);
    const rel = F > 0 ? Math.round(qty / F * 1000) / 10 : 0;
    out.push({ name, units: qty, avgPerSku: null, skus: null, normShare: null, rel, qty, lowData: false, efficient: false, custom: true, mode, value });
  }
  return out;
}
// Дополнить результат долей по цветам «моими цветами» (в ядро ассортимента, пересчёт тиража).
function mergeCustomColors(cq, article, forecast) {
  if (!cq) return cq;
  const existing = new Set([...cq.core, ...cq.weak].map((r) => r.name));
  const extra = customColorRows(article, forecast, existing);
  if (!extra.length) return cq;
...
// лучшего цвета (см. seasonColorQuantities). «Мои цвета» добавляются сверх выборки.
function seasonColorsBlock(rep, p) {
  ...
  if (cq) { cq.core.forEach((r) => qtyByName.set(r.name, { qty: r.qty, rel: r.rel, core: true, lowData: r.lowData, efficient: r.efficient, custom: !!r.custom })); cq.weak.forEach((r) => qtyByName.set(r.name, { qty: r.qty, rel: r.rel, core: false, lowData: r.lowData, efficient: r.efficient, custom: false })); }

  // Псевдо-цвета для «моих цветов» (их нет в ca.colors) — чтобы показать в таблице.
  const customCells = customRows.map((r) => ({ name: r.name, skus: null, units: r.qty, rawShare: null, avgPerSku: null, lowConfidence: false, custom: true }));
  ...
  const ordered = (cq
    ...
    .concat(customCells); // «мои цвета» — в конце списка
  ...
  const usedCustom = new Set(ccList.map((cc) => String((cc && cc.name) || '')));
  const availForNew = article ? activeColors(article).filter((nm) => !analyzedNames.has(nm) && !usedCustom.has(nm)) : [];
  const ccRowsHtml = ccList.map((cc, i) => {
    const name = String((cc && cc.name) || '');
    const mode = cc.mode === 'qty' ? 'qty' : 'pct';
    const value = Math.max(0, +cc.value || 0);
    const qty = mode === 'qty' ? Math.round(value) : Math.round(forecast * value / 100);
    const valid = article && activeColors(article).includes(name);
    const opts = [...new Set([name, ...availForNew].filter(Boolean))];
    return `<div class="se-cc-row">
      <select class="se-cc-name" data-cc-idx="${i}">${name ? '' : '<option value="" selected>— выбери цвет —</option>'}${opts.map((nm) => `<option value="${seEsc(nm)}"${nm === name ? ' selected' : ''}>${seEsc(nm)}</option>`).join('')}</select>
      <select class="se-cc-mode" data-cc-idx="${i}"><option value="pct"${mode === 'pct' ? ' selected' : ''}>% от лидера</option><option value="qty"${mode === 'qty' ? ' selected' : ''}>штук</option></select>
      <input class="se-cc-value" type="number" min="0" step="${mode === 'pct' ? '5' : '50'}" data-cc-idx="${i}" value="${value}" style="width:80px">
      <span class="mini">= <b>${fmt(qty)}</b> шт${!valid ? ' <span style="color:var(--danger)">цвет не найден у артикула</span>' : ''}</span>
      <button class="se-cc-del" data-cc-idx="${i}" title="убрать">✕</button>
    </div>`;
  }).join('');
  const customEditor = article ? `<div class="se-custom">
      <div class="mini"><b>🧵 Мои цвета</b> — расцветки, которых нет в рыночной выборке, но которые ты уже продаёшь/шьёшь. Задай объём (доля от лидера или штуки) — цвет войдёт в «План по размерам», партии и поставки наравне с остальными. Список цветов берётся из карточки артикула («Данные»).</div>
      ${ccRowsHtml || '<div class="mini">Пока нет. Нажми «＋ добавить свой цвет».</div>'}
      <button class="btn btn-subtle" id="se-cc-add" type="button"${availForNew.length ? '' : ' disabled title="Нет свободных цветов артикула — добавь новый цвет в «Данные»"'}>＋ добавить свой цвет</button>
    </div>` : '';
...
function reconcileInputs(rep, p, articleId) {
...
function reconcileEffectiveArticle(article, choices, newNames) {
  ...
  // «Мои цвета» = сразу цвет карточки: само-привязка, чтобы reconcilePlan сматчил их напрямую.
  for (const cc of (article.customColors || [])) {
    const nm = String((cc && cc.name) || '').trim();
    if (nm && colors.includes(nm)) colorMap[nm] = nm;
  }
...
// «Мои цвета»: добавить / сменить цвет / режим / значение / убрать. Каждое действие пишет в
// article.customColors, помечает dirty и перерисовывает сезонку (кол-ва, reconcile и планы —
// с учётом новых цветов). Сохранение — общей кнопкой «Сохранить» (state PUT).
function bindSeasonCustomColors(rep, p, articleId) {
  const article = state.articles.find((x) => x.id === articleId);
  if (!article) return;
  const list = () => (article.customColors = Array.isArray(article.customColors) ? article.customColors : []);
  const rerender = () => { dirty = true; setStatus(); renderSeasonView(articleId); };
  document.getElementById('se-cc-add')?.addEventListener('click', () => {
    const analyzed = new Set(((rep.colorAnalysis && rep.colorAnalysis.colors) || []).map((c) => c.name));
    const used = new Set(list().map((cc) => String((cc && cc.name) || '')));
    const avail = activeColors(article).filter((nm) => !analyzed.has(nm) && !used.has(nm));
    list().push({ name: avail[0] || '', mode: 'pct', value: 50 });
    rerender();
  });
  document.querySelectorAll('.se-cc-name').forEach((el) => el.addEventListener('change', (e) => {
    const i = +e.target.dataset.ccIdx; if (list()[i]) { list()[i].name = e.target.value; rerender(); }
  }));
  document.querySelectorAll('.se-cc-mode').forEach((el) => el.addEventListener('change', (e) => {
    const i = +e.target.dataset.ccIdx; if (list()[i]) { list()[i].mode = e.target.value === 'qty' ? 'qty' : 'pct'; rerender(); }
  }));
  document.querySelectorAll('.se-cc-value').forEach((el) => el.addEventListener('change', (e) => {
    const i = +e.target.dataset.ccIdx; if (list()[i]) { list()[i].value = Math.max(0, +e.target.value || 0); rerender(); }
  }));
...
let pcMetric = 'mixed';      // 'buyouts' | 'mixed' | 'revenue' — метрика ранжирования «слабых»
const pcKey = (r) => r.articleId + '|' + r.color;
const pcIsProtected = (r) => !!(state.articles.find((a) => a.id === r.articleId) || {}).cutProtected;
// «сила» расцветки (больше = ценнее, режем последней). Для 'mixed' — блендим выкупы и выручку,
// нормируя обе величины к 0..1 по максимуму среди кандидатов (разные цены → честнее в долях).
function pcScorer(rows, metric) {
  const cand = rows.filter((r) => r.matched);
  const maxB = Math.max(1, ...cand.map((r) => r.buyouts || 0));
  const maxR = Math.max(1, ...cand.map((r) => r.forPay || 0));
  return (r) => {
    if (metric === 'buyouts') return r.buyouts || 0;
    if (metric === 'revenue') return r.forPay || 0;
    return ((r.buyouts || 0) / maxB) * 0.5 + ((r.forPay || 0) / maxR) * 0.5; // mixed 50/50
  };
}
// жадный выбор: режем самых слабых по метрике (при равных — где больше штук плана) до цели.
// Новинки/защищённые артикулы (a.cutProtected) в авто-рез НЕ попадают.
function pcGreedy(rows, planTotal, target, metric) {
  const need = Math.max(0, planTotal - target);
  const score = pcScorer(rows, metric || pcMetric);
  const cand = rows.filter((r) => r.matched && !pcIsProtected(r)).slice()
    .sort((a, b) => (score(a) - score(b)) || (b.planUnits - a.planUnits));
...
async function pcLoadAnalysis() {
  ...
  if (plancutAnalysis && plancutAnalysis.rows && pcCutSel === null) pcCutSel = pcGreedy(plancutAnalysis.rows, plancutAnalysis.planTotal, pcTarget, pcMetric);
  if (activeTab === 'plancut') renderPlanCut();
}
// сохранить state (защита артикулов) без пересборки карты; затем перерисовать
function pcPersistLite() {
  dirty = true; setStatus();
  clearTimeout(pcSaveT);
  pcSaveT = setTimeout(async () => {
    try { await api('/api/state', { method: 'PUT', body: JSON.stringify(state) }); dirty = false; setStatus(); }
...
function renderPlanCut() {
  ...
  if (!root) return;
  const nf = (n) => (Math.round(+n || 0)).toLocaleString('ru');
  ...
  if (an === undefined) cutPanel = cutHead + '<div class="mini">Считаю разбор плана…</div></div>';
    ...
    if (pcCutSel === null) pcCutSel = pcGreedy(an.rows, an.planTotal, pcTarget, pcMetric);
    // текущий срез (учёт ручных правок), защищённые не режем даже вручную по ошибке
    let removed = 0, cutCount = 0, protUnits = 0;
    for (const r of an.rows) {
      if (pcIsProtected(r)) { protUnits += r.planUnits; continue; }
      if (pcCutSel.has(pcKey(r))) { removed += r.planUnits; cutCount += 1; }
    }
    ...
    // метрика ранжирования + защита новинок
    const mBtn = (id, lbl, hint) => `<button class="btn${pcMetric === id ? ' btn-primary' : ''}" data-pc-metric="${id}" style="padding:3px 10px;font-size:12px" title="${hint}">${lbl}</button>`;
    const protIds = state.articles.filter((a) => a.cutProtected).map((a) => a.id);
    const artIds = [...new Set(an.rows.map((r) => r.articleId))].sort();
    const protChips = artIds.map((id) => `<label class="sp-chip${protIds.includes(id) ? ' on' : ''}" style="cursor:pointer"><input type="checkbox" data-pc-protect="${seEsc(id)}"${protIds.includes(id) ? ' checked' : ''}> ${seEsc(id)}</label>`).join('');
    const controls = `<div class="panel" style="margin-bottom:0">
      <div style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-start">
        <div><div class="mini" style="margin-bottom:4px">Метрика «слабой» расцветки:</div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            ${mBtn('buyouts', 'Выкупы шт', 'Ранжировать по числу выкупов за 6 мес')}
            ${mBtn('mixed', 'Смешанная 50/50', 'Блендим выкупы и выручку (обе в долях от максимума)')}
            ${mBtn('revenue', 'Выручка ₽', 'Ранжировать по фактической выручке за 6 мес')}
          </div></div>
    ...
    const score = pcScorer(an.rows, pcMetric);
    const cand = an.rows.filter((r) => r.matched).slice()
      .sort((a, b) => (pcIsProtected(a) - pcIsProtected(b)) || (score(a) - score(b)) || (b.planUnits - a.planUnits));
    const row = (r) => {
      const prot = pcIsProtected(r);
      const sel = !prot && pcCutSel.has(pcKey(r));
      const bg = prot ? 'background:#eef6ff' : (sel ? 'background:#fdecea' : '');
      const dead = (r.buyouts || 0) <= 0;
      return `<tr style="${bg}">
        <td style="text-align:center">${prot ? '🛡' : `<input type="checkbox" data-pc-cut="${seEsc(pcKey(r))}"${sel ? ' checked' : ''}>`}</td>
        <td>${seEsc(r.articleId)}${prot ? ' <span class="mini" style="color:#2563a0">новинка</span>' : ''}</td>
        <td>${seEsc(r.color)}</td>
        <td class="num">${nf(r.planUnits)}</td>
        <td class="num"${dead && !prot ? ' style="color:#9b1c1c;font-weight:700"' : ''}>${nf(r.buyouts)}</td>
        <td class="num">${r.forPay != null ? nf(r.forPay) : '—'}</td>
    ...
    const unmRow = (r) => {
      const prot = pcIsProtected(r);
      const sel = !prot && pcCutSel.has(pcKey(r));
      return `<tr style="${prot ? 'background:#eef6ff' : (sel ? 'background:#fdecea' : 'background:#f4f5f7')}">
        <td style="text-align:center">${prot ? '🛡' : `<input type="checkbox" data-pc-cut="${seEsc(pcKey(r))}"${sel ? ' checked' : ''}>`}</td>
        <td>${seEsc(r.articleId)}${prot ? ' <span class="mini" style="color:#2563a0">новинка</span>' : ''}</td>
        <td>${seEsc(r.color)} <span class="mini">(не сшито)</span></td><td class="num">${nf(r.planUnits)}</td></tr>`;
    };
  ...
  // смена метрики — пересчитать жадный выбор под новую метрику
  root.querySelectorAll('[data-pc-metric]').forEach((el) => el.addEventListener('click', () => {
    pcMetric = el.dataset.pcMetric;
    pcCutSel = pcGreedy(plancutAnalysis.rows, plancutAnalysis.planTotal, pcTarget, pcMetric);
    renderPlanCut();
  }));
  // защита артикула (новинка) — сохранить в state, убрать из реза, пересчитать
  root.querySelectorAll('[data-pc-protect]').forEach((el) => el.addEventListener('change', () => {
    const a = state.articles.find((x) => x.id === el.dataset.pcProtect); if (!a) return;
    a.cutProtected = el.checked;
