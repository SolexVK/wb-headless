...
включи Funnel **в фоновом режиме** (ключ `--bg` обязателен):

```bash
tailscale funnel --bg 8090
tailscale funnel status   # проверка: покажет https://…ts.net → http://localhost:8090
```

Он выдаст постоянную https-ссылку вида `https://mac-mini.ваш-tailnet.ts.net`
(имя не меняется — это имя машины в tailnet).
**Обязательно задай `PLANNER_PASSWORD`** — по этой ссылке инструмент
доступен из интернета.

> ⚠️ **Только `--bg`!** Без него (`tailscale funnel 8090`) туннель живёт лишь
> пока открыт терминал и НЕ поднимается после перезагрузки Mac mini —
> отсюда `ERR_CONNECTION_CLOSED`. С `--bg` конфиг сохраняется, и системный
> демон `tailscaled` (установленный `install-system-daemon`) сам поднимает
> Funnel при каждом старте.

**Чтобы после ребута вставало всё автоматически:**
1. `tailscaled` — системный демон (уже сделано `sudo tailscaled install-system-daemon`),
   стартует до логина.
2. Funnel — включён с `--bg` (см. выше), конфиг переживает перезагрузку.
3. Планировщик — launchd-агент `com.wbheadless.planner` с `RunAtLoad`+`KeepAlive`,
   но это **агент пользователя**: он стартует только после входа в macOS. На headless
   Mac mini включи **автологин**: Системные настройки → Пользователи и группы →
   «Автоматический вход» → твой пользователь. Тогда после ребута сессия входит сама,
   агент поднимается, а вместе с ним и сервер.
