...
// Токен берём из окружения (planner/server.js грузит data/.env в process.env).
function resolveWbToken() {
  const token = process.env.WB_API_TOKEN || process.env.Wildberries_API || null;
  return { token, source: token ? (process.env.WB_API_TOKEN ? "env WB_API_TOKEN" : "env Wildberries_API") : null };
}
function resolveTokenType(t) { return t || "personal"; }
