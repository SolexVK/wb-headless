 ...
 * меньше минимальной производственной партии — сливаем его НАЗАД, в ПРЕДЫДУЩИЙ по сроку довоз
 * того же артикула (накопительно). Мутирует `net` (обнуляет слитый довоз, наращивает предыдущий).
 *
 * ПОЧЕМУ НАЗАД, а не вперёд: слить вперёд = произвести/привезти остаток ПОЗЖЕ, чем планировали —
 * это риск дефицита в раннем сезоне (осенний остаток уезжает в зимнюю поставку). Слить назад =
 * привезти ЧУТЬ РАНЬШЕ (безопасно, лишь связывает капитал). Поэтому мелкий нетто прибавляем к
 * ближайшему БОЛЕЕ РАННЕМУ довозу.
 *
 * Не трогаем: ПЕРВЫЙ довоз (слить назад некуда — остаётся как есть) и ПОСЛЕДНИЙ/хвост (по желанию
 * пользователя «последняя партия может быть маленькой»). Т.е. сливаются только СРЕДНИЕ мелкие.
...
export function mergeSmallNetDeliveries(state, net, minBatch) {
  ...
  for (const aid in byArt) {
    ...
    // идём с конца к началу (кроме хвоста и первого): мелкий средний довоз прибавляем к предыдущему;
    // предыдущий за счёт этого растёт — следующая (более ранняя) итерация это увидит (цепочка назад).
    for (let i = parts.length - 2; i >= 1; i--) {
      const p = parts[i];
      const u = sumSupplyMatrix(net[p.id]);
      if (u > 0 && u < min) {                 // мал → сливаем в ПРЕДЫДУЩИЙ (более ранний) довоз
        const pv = parts[i - 1];
        net[pv.id] = net[pv.id] || {};
        addMatrixInto(net[pv.id], net[p.id]);
        merges.push({ articleId: aid, from: p.id, into: pv.id, units: u });
