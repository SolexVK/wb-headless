...
export function applyOOSCorrection(daily) {
  ...
  const avgPrice = saleDays.length
    ...
    : 0;
  // ВАЖНО: восстанавливаем спрос ТОЛЬКО внутри активного периода карточки (между
  // первым и последним днём с продажами/остатком). Дни ДО запуска / ПОСЛЕ снятия
  // (в графике категории это нули) — не «простой», товара тогда не существовало,
  // фабриковать там спрос нельзя (иначе профиль сезона искажается).
  let first = -1, last = -1;
  for (let i = 0; i < daily.length; i++) {
    if ((daily[i].sales || 0) > 0 || (daily[i].balance || 0) > 0) { if (first < 0) first = i; last = i; }
  }
  if (first < 0) return daily.map((r) => ({ ...r }));
  return daily.map((r, i) => {
    if (i >= first && i <= last && (r.balance || 0) <= 0 && (r.sales || 0) <= 0) {
...
export function computeFoldedMonthlyProfile(active, opts = {}) {
  ...
  // Циклическое сглаживание месячного профиля (гасит одномесячные launch-спайки:
  // осенние новинки в августе раздувают месяц; сезонность должна быть плавной).
  if (opts.foldSmooth !== false && months.length >= 3) {
    const im = {}, pm = {};
    for (const m of months) { im[m.month] = m.index; pm[m.month] = m.priceIndex; }
    const near = (map, mo) => { const c = map[mo]; const p = map[mo === 1 ? 12 : mo - 1] ?? c; const n = map[mo === 12 ? 1 : mo + 1] ?? c; return 0.25 * p + 0.5 * c + 0.25 * n; };
    for (const m of months) { m.indexRaw = m.index; m.priceIndexRaw = m.priceIndex; m.index = round(near(im, m.month), 3); m.priceIndex = round(near(pm, m.month), 3); }
  }
...
export function buildSeasonPlan(groupDaily, opts = {}) {
  ...
  const daily = planDailyOrders(coeffs.dates, coeffs.kSales, baseDaily, weekdayFactors).map((d, i) => ({
    ...
    stock: round(Number(active[i]?.stock ?? active[i]?.balance) || 0, 0), // остатки группы
