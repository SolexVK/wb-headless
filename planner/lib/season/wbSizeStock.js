...
const DETAIL_URL = process.env.WB_CARD_DETAIL_URL || 'https://card.wb.ru/cards/v2/detail';
...
async function fetchChunk(nmIds) {
 ...
 */
export async function fetchSizeStock(nmIds) {
  const ids = [...new Set((nmIds || []).map(Number).filter((n) => Number.isFinite(n) && n > 0))];
  const fx = fixture();
  if (fx) return parseDetail(fx); // фикстура покрывает столько товаров, сколько в файле
  const out = new Map();
