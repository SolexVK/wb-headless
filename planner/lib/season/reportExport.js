...
/**
 * SVG-диаграмма с сеткой и подписями обеих осей. primary — площадь+линия (левая
 * ось), secondary — пунктир (правая ось). Ключи и единицы настраиваются, чтобы
 * рисовать и «заказы/цена», и «остатки/продажи». smoothPrimary — сгладить пилу.
 */
function svgChart(daily, opts) {
  const {
    primaryKey = 'plannedOrders', primaryLabel, primaryColor, primaryUnit = 'шт',
    secondaryKey = 'price', secondaryLabel, secondaryColor, secondaryUnit = '₽',
    tertiaryKey, tertiaryLabel, tertiaryColor, tertiaryUnit = 'шт',
    smoothPrimary = false, smoothSecondary = false,
  } = opts;
  const hasT = !!tertiaryKey && daily.some((r) => Number(r[tertiaryKey]) > 0);
  const n = daily.length || 1;
  const W = 1000, H = 360, padL = 62, padR = hasT ? 112 : 64, padT = 22, padB = 58;
  const plotW = W - padL - padR, plotH = H - padT - padB;
  const x = (i) => padL + (plotW * i) / Math.max(1, n - 1);
  const rightInner = padL + plotW, rightOuter = padL + plotW + 48;

  let pv = daily.map((r) => Number(r[primaryKey]) || 0);
  ...
  const isMoney = secondaryUnit === '₽';
  const minS = isMoney ? Math.floor(minSraw / 100) * 100 : 0;
  const maxS = (isMoney ? Math.ceil(maxSraw / 100) * 100 : niceMax(maxSraw)) || 1;
  let tv = hasT ? daily.map((r) => Number(r[tertiaryKey]) || 0) : [];
  if (hasT && smoothSecondary) tv = ma7(tv);
  const maxT = hasT ? niceMax(Math.max(1, ...tv)) : 1;

  const yP = (v) => padT + plotH - (plotH * v) / maxP;
  const yS = (v) => padT + plotH - (plotH * (v - minS)) / Math.max(1, maxS - minS);
  const yT = (v) => padT + plotH - (plotH * v) / maxT;
...
<rect x="${x0.toFixed(1)}" y="${padT}" width="${Math.max(0.6, x1 - x0).toFixed(1)}" height="${plotH}" fill="${FAVORABLE}" opacity="0.14"/>`;
  ...
  // Сетка + подписи: левая (спрос), внутр. правая (цена), внеш. правая (остатки).
  const grid = [];
  const steps = 4;
  for (let g = 0; g <= steps; g++) {
    ...
    grid.push(`<line x1="${padL}" y1="${yy.toFixed(1)}" x2="${rightInner}" y2="${yy.toFixed(1)}" stroke="var(--line)" stroke-width="1"/>
<text x="${padL - 6}" y="${(yy + 3).toFixed(1)}" font-size="9" text-anchor="end" fill="${primaryColor}">${fmt(ov)}</text>
<text x="${rightInner + 6}" y="${(yy + 3).toFixed(1)}" font-size="9" fill="${secondaryColor}">${fmt(pvp)}</text>` +
      (hasT ? `<text x="${rightOuter + 6}" y="${(yy + 3).toFixed(1)}" font-size="9" fill="${tertiaryColor}">${fmt(Math.round((maxT * (steps - g)) / steps))}</text>` : ''));
  }
  ...
  let secS = daily.map((r) => Number(r[secondaryKey]) || 0);
  if (smoothPrimary || smoothSecondary) secS = ma7(secS);
  const linePts = daily.map((r, i) => (secS[i] > 0 ? `${x(i).toFixed(1)},${yS(secS[i]).toFixed(1)}` : null)).filter(Boolean).join(' ');
  const line = `<polyline points="${linePts}" fill="none" stroke="${secondaryColor}" stroke-width="1.8" stroke-dasharray="5 3"/>`;
  const stockLine = hasT
    ? `<polyline points="${daily.map((r, i) => `${x(i).toFixed(1)},${yT(tv[i]).toFixed(1)}`).join(' ')}" fill="none" stroke="${tertiaryColor}" stroke-width="1.6" stroke-dasharray="1 3" stroke-linecap="round"/>`
    : '';

  // Недельные тики; подпись даты раз в ~2 недели.
  const tickEls = [];
  let k = 0;
  daily.forEach((r, i) => {
  ...
  // Легенда линий (3, если есть остатки).
  const lg = (dx, color, dash, label) =>
    `<line x1="${padL + dx}" y1="${padT - 8}" x2="${padL + dx + 18}" y2="${padT - 8}" stroke="${color}" stroke-width="2"${dash ? ` stroke-dasharray="${dash}"` : ''}/><text x="${padL + dx + 23}" y="${padT - 5}" fill="var(--fg)">${label}</text>`;
  const legend = `<g font-size="10">
    ${lg(0, primaryColor, '', `${primaryLabel}, ${primaryUnit}${smoothPrimary ? ' (сглаж.)' : ''}`)}
    ${lg(240, secondaryColor, '5 3', `${secondaryLabel}, ${secondaryUnit}`)}
    ${hasT ? lg(430, tertiaryColor, '1 3', `${tertiaryLabel}, ${tertiaryUnit}`) : ''}
  </g>`;
...
<p class="sub">Корректировка по конкурентам за ${p.adjustments.windowDays} дн.: цена ${fmt(p.adjustments.priorPrice)}→${fmt(p.adjustments.recentPrice)} ₽, продажи ${p.adjustments.priorAvgDaily}→${p.adjustments.recentAvgDaily} шт/день.</p>` : '';
  ...
  // Единые цвета линий на всех диаграммах: спрос — синий, цена — красный,
  // остатки — фиолетовый.
  const C = { demand: '#2646A0', price: '#C1121F', stock: '#8E7CC3' };
  const charts = isForecast ? `
<h2>Прогноз на ${p.forecastPeriod.from} … ${p.forecastPeriod.to}</h2>
<div class="chart">${svgChart(p.forecastDaily || [], { primaryKey: 'plannedOrders', primaryLabel: 'план заказов', primaryColor: C.demand, primaryUnit: 'шт', secondaryKey: 'price', secondaryLabel: 'цена', secondaryColor: C.price, secondaryUnit: '₽', tertiaryKey: 'stock', tertiaryLabel: 'остатки', tertiaryColor: C.stock, tertiaryUnit: 'шт', smoothPrimary: true })}</div>
<h2>История спроса за 2 года (реальные данные аналогов)</h2>
<div class="chart">${svgChart(p.historyDaily || [], { primaryKey: 'sales', primaryLabel: 'продажи аналогов', primaryColor: C.demand, primaryUnit: 'шт', secondaryKey: 'price', secondaryLabel: 'цена', secondaryColor: C.price, secondaryUnit: '₽', tertiaryKey: 'stock', tertiaryLabel: 'остатки конкурентов', tertiaryColor: C.stock, tertiaryUnit: 'шт', smoothPrimary: false })}</div>`
    : `<div class="chart">${svgChart(p.daily || [], { primaryKey: 'plannedOrders', primaryLabel: 'план заказов', primaryColor: C.demand, primaryUnit: 'шт', secondaryKey: 'price', secondaryLabel: 'цена', secondaryColor: C.price, secondaryUnit: '₽', tertiaryKey: 'stock', tertiaryLabel: 'остатки', tertiaryColor: C.stock, tertiaryUnit: 'шт', smoothPrimary: false })}</div>`;
