...
На выходе — многоблочный отчёт, из которого напрямую строится план доработки карточки:
  ...
  C. Цена × юнит-экономика: коридор по марже, безубыточность, ценовые сегменты (при --cost)
  D. Доминирующие цвета (базовые): доля выручки/артикулы/сумма заказов по всей базе
  E. Контент-бенчмарк топа: фото/видео/3D/описание/характеристики/рейтинг/отзывы/заказы/выручка
  F. Листинг топов (слайды) + F2. Готовые данные (смыслы · характеристики топов с доминантой)
  F3. Принадлежность признаков топа (авто по card.json): % присутствия, ≥30% = обязателен
  G. Разбор карточки (--my-sku): гэпы против медианы/лидеров ниши (--foreign — чужой артикул)
  G2. Сравнение карточек — воронка (при --funnel-json из [2]; нет при --foreign)
  H. План доработки карточки: цифры-цели + ручные задачи (✅-якоря к авто-блокам)
...
BASE_COLOR_RULES = [
    ("бирюз", "бирюзовый"), ("ментол", "бирюзовый"), ("морск волн", "бирюзовый"),
    ("голуб", "голубой"), ("васил", "голубой"), ("небесн", "голубой"),
    ("индиго", "синий"), ("джинс", "синий"), ("морск", "синий"), ("син", "синий"),
    ("молочн", "белый"), ("белоснеж", "белый"), ("бел", "белый"),
    ("телесн", "бежевый"), ("песочн", "бежевый"), ("крем", "бежевый"), ("ванил", "бежевый"),
    ("кофейн", "бежевый"), ("капучин", "бежевый"), ("латте", "бежевый"), ("беж", "бежевый"),
    ...
    ("шоколад", "коричневый"), ("мокко", "коричневый"), ("кофе", "коричневый"), ("коричн", "коричневый"),
...
def build_report(args, path, path_note, name_filter, items, total, agg, tot_rev,
    ...
    L += ["", "### Ручные задачи (по методике — то, что не даёт авто-анализ)",
...
def main():
    ...
    ap.add_argument("--cards", type=int, default=6,
                    help="размер пула топ-карточек для листинга/характеристик/принадлежности (F/F2/F3)")
