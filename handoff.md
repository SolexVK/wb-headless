...
- **ПОСЛЕДНЕЕ (эта сессия):** (1) фикс сумм («сироты», коммит `664a7fa`); (2) образцы ткани
  на диск + DnD-перестановка цветов (`ae2a2d9`); (3) импорт/экспорт плана через .xlsx (`94ff991`).
  Всё запушено. По вопросу «единый vs по-этапно» — подтверждено: **по-этапно ВЕРНО** (4 этапа =
  4 разных объёма под пары месяцев); планировщик уже изолирует этапы и не залезает в следующий.
...
## Следующий шаг
Дождаться обратной связи пользователя после обновления прода (проверка образцов-миграции,
DnD-цветов, .xlsx-импорта). Затем — следующая правка. Блокеров нет.
