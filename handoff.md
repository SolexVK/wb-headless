...
- Текущий `tailscale serve status` (всё tailnet-only):
  - `:443 /` → 18789 (OpenClaw UI, приватно), `/fbs` → 9000/fbs, `/calc` → 9000/calc
  - `:8443` → 8090 (UNIT-калькулятор), `:10000` → 8787 (китай-калькулятор),
    `:8444` → 8477 (Производственный план)
- **ДОСТУП РЕШЁН:** проблема была в системном прокси Windows (см. «Что НЕ сработало» п.6) —
  добавлен bypass `*.ts.net;100.*`, все сервисы открываются во всех браузерах.
...
6. **ИСТИННЫЙ КОРЕНЬ (найден позже) — СИСТЕМНЫЙ ПРОКСИ на Windows режет НЕ-443 порты.**
   Долго грешили на Chrome/QUIC — НЕВЕРНО. Решающая ошибка в браузере на нестандартных портах
   была **`ERR_TUNNEL_CONNECTION_FAILED`**. У пользователя в Windows прописан прокси
   `200.10.39.195:8000` (для WB/BIFIT), который **пропускает только :443**, а `:8443/:8444/
   :10000` режет. `curl` прокси не использует → работает на всех портах; браузер использует →
   рвёт не-443. **Фикс:** добавить tailnet в исключения прокси. Windows: `inetcpl.cpl` →
   Подключения → Настройка сети → Дополнительно → «Не использовать прокси для…» дописать
   **`*.ts.net;100.*`**. После этого ОТКРЫЛИСЬ ВСЕ сервисы (планировщик :8444, калькуляторы
   :8443/:10000), и Chrome тоже (tailnet пошёл напрямую, как curl). Реестр:
   `HKCU\...\Internet Settings\ProxyOverride` теперь содержит `*.ts.net`.
   Побочно был и вторичный эффект Chrome (на :443 Яндекс открывал, Chrome — нет) — но главный
   барьер был именно прокси. Урок: `ERR_TUNNEL_CONNECTION_FAILED` = прокси, а не сервер/браузерный кэш.

7. **Провал варианта «планировщику отдельный порт :8444».** Технически serve-маршрут
   `:8444 → 8477` встал и curl отдавал `HTTP/2 302 → /login`, но браузер за прокси не мог
   достучаться до :8444 (тот же прокси-барьер, п.6). После исключения прокси — заработало.
   Планировщик (planner/, ветка production-plan) — Telegram-авторизация (`/` → 302 `/login`,
   виджет Telegram проверяет домен → «Bot domain invalid» при заходе по голому IP; по
   hostname `…ts.net` домен совпадает). `/admin` — это админ-страница ПЛАНИРОВЩИКА, не FBS.

8. **Мелочь:** пользователь один раз вставил PowerShell-команду (`curl.exe`, `` `n ``) в Mac zsh —
   zsh завис на `dquote bquote>`. Выход — Ctrl+C. Для Mac-терминала: `curl` и `\n`, не `curl.exe`.
9. **Диагностический приём, который всё решил:** сверять НЕ только код ответа, но и РАЗМЕР/ТЕЛО
   (FBS healthz = 71 байт JSON; оболочка OpenClaw = 10316 байт) и смотреть точный код ошибки
   браузера (`ERR_TUNNEL_CONNECTION_FAILED` → прокси; `ERR_CONNECTION_CLOSED` → серв/кэш/QUIC).
   И проверять в РАЗНЫХ браузерах (Яндекс открывал то, что Chrome — нет).
...
1. **Публично (интернет) или только tailnet?** Сейчас всё `serve` = только устройства в
   Tailscale (пользователь ходит с Windows в tailnet — всё работает после bypass прокси).
   Если понадобится доступ ВНЕШНИМ клиентам без Tailscale — включить `tailscale funnel` на
   443/8443/10000 (но funnel-порты только эти три, все заняты → планировщик пришлось бы
   вешать путём на 443). **Рекомендация:** OpenClaw (`/`) наружу НЕ выставлять.
   На данный момент пользователь разблокирован (tailnet достаточно). Не срочно.
2. **`/admin`** — РЕШЕНО: это админ-страница ПЛАНИРОВЩИКА (planner `/admin` → admin.html),
   а не FBS. Открывается по `…:8444/admin` после входа.
3. **Производственный план** — РЕШЕНО: отдан на `:8444 → 8477`, открывается
   `https://mac-mini-vital.tail13d29e.ts.net:8444/` (после bypass прокси). Вход через Telegram.

## Следующий шаг
Доступ и планировщик — решены (bypass прокси + `:8444`). Осталось только закрепить durable.
**Управляемо активировать durable-фикс OpenClaw**, чтобы апдейт v2026.7.1 не стёр маршруты
(`:8444` планировщика тоже добавлен вручную и слетит при wipe):
1. Точный рестарт гейтвея OpenClaw (НЕ FBS): `openclaw daemon restart` (или его launchd-label
   ...
   `resetOnExit:false`.
2. Сразу после — один раз повторить ВСЕ маршруты (идемпотентно):
   ```
   ...
   tailscale serve --bg --https=8444  http://127.0.0.1:8477
   ```
3. Проверить `tailscale serve status` — все 5 маршрутов на месте.
Напоминание про доступ с любого НОВОГО устройства за прокси: добавить в исключения прокси
`*.ts.net;100.*` (иначе не-443 порты не откроются — см. «Что НЕ сработало» п.6).
