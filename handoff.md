...
## Следующий шаг
Дождаться от пользователя подтверждения публичного доступа с **телефона** (мобильный интернет):
`https://mac-mini-vital.tail13d29e.ts.net/` (planner), `.../calc` (tandemtrace),
`:8443/` (getcourse), `:10000/` (наш калькулятор). Если `/calc` открывается публично — инцидент
полностью закрыт. Если что-то из соседей не отвечает — свериться с `tailscale funnel status`
и `brew services list | grep caddy`, при нужде `caddy reload`.

Дальше — вернуться к основной задаче калькулятора: взять параметры первого реального товара
(закупка ¥, вес, категория, цель по цене, ожидаемый выкуп) и прогнать юнит-расчёт.
Опционально (обсуждали, не срочно): завести getcourse (8443) и наш калькулятор (10000) тоже за
Caddy на 443 по путям, освободив выделенные порты — но это меняет их публичные URL, только по согласованию.
