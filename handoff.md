...
1. **Пик-мерж vs строго текущий отчёт** для матрицы NMID×склад. Сейчас пик-мерж
   (включает Коледино и др. обнулённые = верхняя оценка). Альтернатива — только
   текущий remains (тогда обнулённые склады выпадут). Рекомендация: оставить пик-мерж.
   Не критично, ЖДУ ПРИ ЖЕЛАНИИ.
2. **Ежедневный автоснимок** (`report:matrix`/`report:cabinet` по расписанию), чтобы
   зафиксировать потери ДО полного списания WB. Рекомендация: включить. ЖДУ ОТВЕТА.
Прочих открытых развилок нет.

## Следующий шаг
Работа по последнему запросу (Excel+PDF отчёт NMID×склад) завершена и отправлена
пользователю. При возобновлении — по желанию пользователя: настроить ежедневный
автоснимок (cron/Routine) для фиксации потерь до списания, либо доработки форматов
(бренд/предмет в PDF-матрице, иная сортировка/группировка, строго-текущий режим).
