...
/** Считывает счётчик «Карточки для сравнения: N из 5». */
async function readCompareCounter(page) {
  const t = await page
    .getByText(/Карточки для сравнения:\s*(\d+)\s*из\s*5/i)
    .first()
    .textContent()
    .catch(() => '');
  return Number((String(t).match(/(\d+)\s*из/) || [])[1] || 0);
}

/**
 * Очищает черновик сравнения (кнопка «Удалить все карточки»), если он не пуст.
 * Иначе submit после DRY-RUN упирается в заполненную форму (поле ввода скрыто).
 */
async function clearDraft(page, log) {
  if ((await readCompareCounter(page)) === 0) return;
  const btn = page.getByRole('button', { name: /Удалить все карточки/i }).first();
  if (!(await btn.count())) return;
  await btn.click().catch(noop);
  await sleep(page, 1000);
  // возможное подтверждение в модалке (best-effort)
  await page.getByRole('button', { name: /^(Удалить все|Удалить|Да|Подтвердить|Очистить)$/i }).first().click().catch(noop);
  await sleep(page, 1500);
  log(`  очищен предыдущий черновик сравнения (было ${await readCompareCounter(page)} из 5)`);
}
...
export async function runCardsComparison(a = {}) {
  ...
  try {
    ...
    await sleep(page, 1500);
    // Очищаем возможный черновик от прошлого прогона (иначе форма может быть 5/5
    // и поле ввода скрыто → таймаут). Делает прогон идемпотентным.
    await clearDraft(page, log);
