...
/** Ссылки на слайды big/{i}.webp (до limit штук). */
export function slideUrls(card, limit = 5) {
  if (!card) return [];
  const nm = card._nm;
  ...
  return Array.from({ length: count }, (_, i) =>
    `https://basket-${h}.wbbasket.ru/vol${vol}/part${part}/${nm}/images/big/${i + 1}.webp`
  );
}
...
/** Скачивает слайды карточки в dir/<nm>_<i>.webp; возвращает список сохранённых путей. */
export async function downloadSlides(card, dir, limit = 5) {
  const urls = slideUrls(card, limit);
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    const p = `${dir}/${card._nm}_${i + 1}.webp`;
