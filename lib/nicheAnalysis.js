...
function normalizeSearchRow(row) {
  ...
  return {
    ...
    // остаток и оборачиваемость — для sell-through (метод Овчинникова)
...
/**
 * Sell-through (метод Овчинникова): в указанный период соотносим объём продаж
 * с объёмом остатков. Чем меньше остатков и больше продаж — тем выгоднее период
 * (спрос не покрыт предложением). Возвращает {ratio, medianTurnoverDays, ...}.
 *   ratio = Σ продано(шт) / Σ остаток(шт).  >1 — распродают быстрее, чем лежит на складе.
 *   turnover_days (медиана) — за сколько дней распродаётся остаток (меньше = лучше).
 */
export function sellThrough(rows) {
  const units = rows.reduce((s, r) => s + num(r.units ?? r.sales), 0);
  const stock = rows.reduce((s, r) => s + num(r.balance), 0);
...
// Строит заключение о целесообразности входа — на конкретных цифрах.
function buildConclusion(x) {
  ...
  pts.push({
    ...
    text: `Ёмкость ≈ ${Math.round(revMonth / 1e6)} млн ₽/мес; на «живой» товар ${Math.round((x.capacity.avgRevenuePerActiveProduct * k) / 1000)} тыс ₽/мес.`,
  });
  // Sell-through (Овчинников)
  if (x.nicheSellThrough.ratio != null) {
    const r = x.nicheSellThrough.ratio;
    pts.push({
      tone: r >= 1.5 ? 'good' : r >= 0.8 ? 'mid' : 'bad',
      text: `Sell-through ${r}× за период (оборот ${x.nicheSellThrough.medianTurnoverDays} дн.): ${r >= 1.5 ? 'спрос обгоняет остатки — дефицитный, выгодный период' : r >= 0.8 ? 'спрос ≈ предложению' : 'склады переполнены относительно продаж'}.`,
    });
  }
  // Конкуренция
  pts.push({
    tone: x.competition.monopolyPct <= 15 ? 'good' : x.competition.monopolyPct < 40 ? 'mid' : 'bad',
    text: `Монополизация топ-10 = ${round(x.competition.monopolyPct)}% (${x.competition.monopolyPct <= 15 ? 'вход открыт' : x.competition.monopolyPct < 40 ? 'умеренная' : 'высокая — нужен бюджет'}); барьер по отзывам — медиана ${x.competition.medianTopComments}.`,
  });
  // Тренд
  if (x.trend) pts.push({
    tone: x.trend.direction === 'up' ? 'good' : x.trend.direction === 'down' ? 'bad' : 'mid',
    text: `Тренд ${x.trend.growthPct > 0 ? '+' : ''}${x.trend.growthPct}% г/г${x.trend.recentYoyPct != null ? ` (последние 3 мес ${x.trend.recentYoyPct > 0 ? '+' : ''}${x.trend.recentYoyPct}%)` : ''}.`,
  });
  // Сезонность
  ...
  // Лучшая под-фраза по sell-through
  ...
  if (withSt.length) {
    ...
    pts.push({
      tone: best.sellThrough.ratio >= 1.5 ? 'good' : 'mid',
      text: `Лучшая точка входа по sell-through: «${best.query}» — ${best.sellThrough.ratio}× (оборот ${best.sellThrough.medianTurnoverDays} дн.).`,
    });
  }
  return { verdict: x.verdict, total: x.total, bottleneck: x.bottleneck, points: pts };
}
