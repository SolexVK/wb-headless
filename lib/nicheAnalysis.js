 ...
 *   коэф = Σ продано(шт) / Σ средний остаток(шт), ПЕРЕСЧИТАНО НА 30 ДНЕЙ —
 *   иначе на длинном периоде ratio растёт линейно и порог 1.5× теряет смысл.
 *   >1 (в мес) — распродают быстрее, чем лежит на складе.
 *   оборачиваемость (медиана turnover_days) — период-независима, за сколько дней уходит остаток.
 */
export function sellThrough(rows, periodDays = 30) {
  ...
  const k = 30 / (periodDays || 30); // нормировка к месяцу
  return {
    ratio: stock > 0 ? round((units / stock) * k, 2) : null,
...
const MONTH_ABBR = { '01': 'янв', '02': 'фев', '03': 'мар', '04': 'апр', '05': 'май', '06': 'июн', '07': 'июл', '08': 'авг', '09': 'сен', '10': 'окт', '11': 'ноя', '12': 'дек' };

/**
 * Временнóй ряд реализации остатка по месяцам за период: агрегирует дневные ряды
 * продаж/остатков (sales_graph/stocks_graph) по всем товарам → помесячно
 * реализация = Σпродажи месяца / средний остаток месяца. >1 — спрос выше
 * предложения (благоприятное окно для продаж).
 */
export function sellThroughTimeline(items, d1, d2) {
  const rows = items.filter((r) => Array.isArray(r.salesGraph) && Array.isArray(r.stocksGraph));
  if (!rows.length) return null;
  const len = Math.max(...rows.map((r) => Math.min(r.salesGraph.length, r.stocksGraph.length)));
  if (!len) return null;
  const sales = new Array(len).fill(0);
  const stock = new Array(len).fill(0);
  for (const r of rows) {
    for (let i = 0; i < len; i++) {
      sales[i] += Number(r.salesGraph[i]) || 0;
      stock[i] += Number(r.stocksGraph[i]) || 0;
    }
  }
  const start = Date.parse(`${d1}T00:00:00Z`);
  if (Number.isNaN(start)) return null;
  const byMonth = new Map();
  for (let i = 0; i < len; i++) {
    const key = new Date(start + i * 86400000).toISOString().slice(0, 7);
    const m = byMonth.get(key) || { sales: 0, stockSum: 0, days: 0 };
    m.sales += sales[i];
    m.stockSum += stock[i];
    m.days += 1;
    byMonth.set(key, m);
  ...
  return [...byMonth.entries()]
    ...
    .map(([ym, m]) => {
      const avgStock = m.days ? m.stockSum / m.days : 0;
      const ratio = avgStock > 0 ? round(m.sales / avgStock, 2) : null;
      return {
        month: ym,
        label: `${MONTH_ABBR[ym.slice(5, 7)]} ${ym.slice(2, 4)}`,
        ratio,
        favorable: ratio != null && ratio > 1,
...
export async function buildQueryDemand(
  ...
  return {
    ...
    ratioVerdict,
    // реализация остатка по ТОП-выдаче запроса — из реальных продаж/остатков (на месяц)
    sellThrough: sellThrough(items, periodDays(period.d1, period.d2)),
    notes,
  };
}
...
// Выручка/продажи/остаток ТОП 1-3 карточек в ценовом диапазоне (наша цель — выйти в ТОП,
// значит ориентируемся не на среднюю карточку, а на лидеров сегмента). Нормировка на месяц.
function topInBand(items, from, to, days = 30, n = 3) {
  if (!Array.isArray(items) || !items.length) return null;
  const k = 30 / (days || 30);
  const inBand = items.filter((r) => r.price >= from && r.price <= to && (r.revenue > 0 || r.units > 0));
  if (!inBand.length) return null;
  const top = inBand.slice().sort((a, b) => b.revenue - a.revenue).slice(0, n);
  const avg = (f) => top.reduce((s, r) => s + (Number(f(r)) || 0), 0) / top.length;
  return {
    topN: top.length,
    inBand: inBand.length,
    topRevenueMonth: round(avg((r) => r.revenue) * k, 0), // ₽/мес у ТОП 1-3 (наша цель)
    topSalesMonth: round(avg((r) => r.units) * k, 0), // шт/мес у ТОП 1-3
    topAvgStock: round(avg((r) => r.avgStock), 0), // средний остаток ТОП 1-3 (реальный ориентир)
  };
}

const ECON = { spp: 0.04, commission: 0.357, acquiring: 0.047, tax: 0.02, drrSteady: 0.08, defect: 0.025 };
export function unitEconomics(cost, medianPrice, e = ECON, segments = null, days = 30, items = null) {
  ...
  const targetPrice = round(priceForMargin(0.30), 0); // цена под целевую маржу 30%

  // Достижимый объём при целевой цене: берём ценовой сегмент вокруг targetPrice.
  // Ориентир — ТОП 1-3 карточки сегмента (мы стремимся в ТОП), а не средняя.
  let target = { price: targetPrice, marginPct: 30 };
  if (Array.isArray(segments) && segments.length) {
    const k = 30 / (days || 30);
    const seg = segments.find((s) => targetPrice >= s.minPrice && targetPrice <= s.maxPrice)
      || segments.slice().sort((a, b) => Math.abs((a.minPrice + a.maxPrice) / 2 - targetPrice) - Math.abs((b.minPrice + b.maxPrice) / 2 - targetPrice))[0];
    if (seg && seg.items) {
      target.segFrom = seg.minPrice;
      target.segTo = seg.maxPrice;
      target.competitors = seg.items;
      const top = topInBand(items, seg.minPrice, seg.maxPrice, days);
      if (top) {
        target.topN = top.topN;
        target.salesPerCardMonth = top.topSalesMonth; // шт/мес у ТОП 1-3
        target.revenuePerCardMonth = top.topRevenueMonth; // ₽/мес у ТОП 1-3
        target.profitPerCardMonth = round(top.topSalesMonth * profitAt(targetPrice), 0); // прибыль ₽/мес при выходе в ТОП
        target.topAvgStock = top.topAvgStock; // средний остаток ТОП 1-3
      } else {
        // запасной вариант — средняя карточка сегмента (если товаров в диапазоне нет)
        target.salesPerCardMonth = round(((seg.sales || 0) / seg.items) * k, 0);
        target.revenuePerCardMonth = round((seg.revenue / seg.items) * k, 0);
        target.profitPerCardMonth = round(target.salesPerCardMonth * profitAt(targetPrice), 0);
      }
      target.salesMonth = round((seg.sales || 0) * k, 0); // весь сегмент, шт/мес
    }
  }
  return {
    cost,
...
// Перспективные ценовые сегменты: высокая выручка на карточку при умеренной
// конкуренции. Возвращает несколько (для дорогих сегментов — приоритет выше цене).
function segmentOpportunity(segments, days = 30, items = null) {
  const k = 30 / (days || 30);
  const segs = segments.filter((s) => s.items > 0 && s.revenue > 0)
    .map((s) => {
      // Выручка/продажи ТОП 1-3 в сегменте — цель при выходе в ТОП (не средняя карточка).
      const top = topInBand(items, s.minPrice, s.maxPrice, days);
      return {
        from: s.minPrice, to: s.maxPrice, items: s.items,
        revenue: round(s.revenue, 0),
        revenuePerCardMonth: round((s.revenue / s.items) * k, 0), // средняя карточка (для справки)
        topRevenueMonth: top ? top.topRevenueMonth : round((s.revenue / s.items) * k, 0), // ТОП 1-3
        topSalesMonth: top ? top.topSalesMonth : round(((s.sales || 0) / s.items) * k, 0),
        topAvgStock: top ? top.topAvgStock : null,
        topN: top ? top.topN : 0,
        salesMonth: round((s.sales || 0) * k, 0),
  ...
  // возможности: конкуренция ≤ медианы, ранжируем по выручке ТОП 1-3 /мес;
  // среди них поднимаем более дорогие сегменты (нам интересна не массовость).
  const medItems = [...segs].map((s) => s.items).sort((a, b) => a - b)[Math.floor(segs.length / 2)];
  const opps = segs.filter((s) => s.items <= medItems && s.topRevenueMonth > 0)
    .sort((a, b) => b.topRevenueMonth - a.topRevenueMonth)
    .slice(0, 3);
  return { topByRevenue: byRevenue, opportunities: opps, opportunity: opps[0] || byRevenue, segments: segs };
}

// Мини-экспресс конкурентный анализ: планка входа по топ-N (что нужно перебить).
function adContentBar(items, topN = 50) {
  ...
  return {
    analyzed: top.length,
    // сколько продаёт топовая карточка в месяц — цель по объёму
    targetSalesMonth: round(med(top.map((r) => r.salesPerDay)) * 30, 0),
    ...
    // барьер соц-доказательства — сколько отзывов у типичной топ-карточки
    medianComments: round(med(top.map((r) => r.comments)), 0),
    medianRating: round(med(top.map((r) => r.rating)), 1),
    // размерный ряд в наличии — типичная широта у топа
    medianSizes: round(med(top.map((r) => r.sizeInStock || r.sizeCount)), 0),
    // длина описания — планка по SEO-тексту
    medianDescLen: round(med(top.map((r) => r.descriptionLength)), 0),
  };
}
...
export async function buildNicheAnalysis({
  ...
  if (days > 62) {
    notes.push(
      `Период длинный (${days} дн.): «% товаров с продажами» кумулятивен и стремится к 100%; ` +
        'реализация пересчитана на месяц. Для «живого» среза спроса берите ~30 дней.'
    );
  }
  ...
  // Реализация остатка ниши (метод Овчинникова): продажи ÷ средний остаток, на месяц.
  const nicheSellThrough = sellThrough(base.items, days);
  // Временнóй ряд реализации по месяцам — благоприятные окна для продаж.
  const sellThroughByMonth = sellThroughTimeline(base.items, d1, d2);
  ...
  let verdict = total >= 70 ? 'Заходить' : total >= 45 ? 'Заходить осторожно' : 'Не заходить';
  ...
  const economics = unitEconomics(cost != null ? Number(cost) : null, t.medianPrice, undefined, segments, days, base.items);
  const buyout = nicheBuyout(base.items);
  const topAges = topAge(base.items, today);
  const priceOpportunity = segmentOpportunity(segments, days, base.items);
  const adContent = adContentBar(base.items);
  const flags = riskFlags({ competition, saturation, trend, buyout, econ: economics });

  // Жёсткие стопы понижают итоговый вердикт — консистентно и в шапке (score.verdict),
  // и в заключении (conclusion.verdict).
  const hardStops = flags.filter((f) => f.level === 'stop');
  if (hardStops.length && verdict === 'Заходить') verdict = 'Заходить осторожно';

  // Топ-продавцы: обогащаем брендом, supplier_id (ссылка на витрину) и выручкой ₽.
  const sellerMeta = new Map();
  for (const it of base.items) {
    const key = it.seller || '(без продавца)';
    if (!sellerMeta.has(key)) sellerMeta.set(key, { supplierId: it.supplierId, brand: it.brand });
  }
  const sellers = (base.sellers || []).map((s) => {
    const m = sellerMeta.get(s.seller) || {};
    return {
      ...s,
      brand: m.brand || '',
      supplierId: m.supplierId || null,
      url: m.supplierId ? `https://www.wildberries.ru/seller/${m.supplierId}` : null,
    };
  });

  // Рекомендация по торговле вне сезона (товар продаётся круглый год).
  const offSeason = seasonality?.sufficient
    ? {
        troughMonth: seasonality.troughMonth,
        troughIndexPct: Math.round((seasonality.months.find((m) => m.month === seasonality.troughMonth)?.index || 0) * 100),
        text:
          `Вне сезона (спад — мес ${seasonality.troughMonth}, рынок ~${Math.round((seasonality.months.find((m) => m.month === seasonality.troughMonth)?.index || 0) * 100)}% от среднего): ` +
          'топы продают круглый год за счёт органики и накопленных отзывов. Держите ассортимент постоянно; ' +
          `в спад — снизьте ДРР и работайте на органике/повторных покупках, копите отзывы и контент к пику (мес ${seasonality.peakMonth}); заходите за 1–1,5 мес до пика.`,
      }
    : null;
  ...
  // Дневные графики больше не нужны в выводе — снимаем, чтобы не раздувать JSON.
  for (const r of base.items) { delete r.salesGraph; delete r.stocksGraph; }
...
function buildConclusion(x) {
  ...
  if (x.nicheSellThrough.ratio != null) {
    ...
    pts.push({
      ...
      text: `Реализация ${r}× (оборот ${x.nicheSellThrough.medianTurnoverDays} дн.): ${r >= 1.5 ? `спрос выше предложения в ${r}× — дефицитный, выгодный период` : r >= 0.8 ? `спрос почти равен предложению (${r}×)` : `предложение выше спроса (${r}×) — склады переполнены`}.`,
    });
  }
  // Конкуренция
  pts.push({
  ...
  if (x.priceOpportunity?.opportunity) {
    ...
    pts.push({ tone: 'good', text: `Перспективный сегмент цены ${o.from}–${o.to} ₽: у ТОП 1-3 ${Math.round((o.topRevenueMonth || o.revenuePerCardMonth) / 1000)} тыс ₽/мес на карточку при ${o.items} конкурентах.` });
  }
