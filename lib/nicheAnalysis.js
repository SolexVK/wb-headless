...
export function unitEconomics(cost, medianPrice, e = ECON, segments = null, days = 30) {
  ...
  if (Array.isArray(segments) && segments.length) {
    ...
    if (seg && seg.items) {
      ...
      target.salesPerCardMonth = round(((seg.sales || 0) / seg.items) * k, 0); // на карточку, шт/мес
      target.revenuePerCardMonth = round((seg.revenue / seg.items) * k, 0); // ₽/мес на карточку
      target.profitPerCardMonth = round(target.salesPerCardMonth * profitAt(targetPrice), 0); // прибыль ₽/мес на карточку
    }
  }
  return {
    cost,
...
function segmentOpportunity(segments, days = 30) {
  ...
  // возможности: конкуренция ≤ медианы, ранжируем по выручке на карточку/мес;
  ...
  const opps = segs.filter((s) => s.items <= medItems && s.revenuePerCardMonth > 0)
    .sort((a, b) => b.revenuePerCardMonth - a.revenuePerCardMonth)
    .slice(0, 3);
...
export async function buildNicheAnalysis({
  ...
  const economics = unitEconomics(cost != null ? Number(cost) : null, t.medianPrice, undefined, segments, days);
  const buyout = nicheBuyout(base.items);
  const topAges = topAge(base.items, today);
  const priceOpportunity = segmentOpportunity(segments, days);
...
function buildConclusion(x) {
  ...
  if (x.priceOpportunity?.opportunity) {
    ...
    pts.push({ tone: 'good', text: `Перспективный сегмент цены ${o.from}–${o.to} ₽: ${Math.round(o.revenuePerCardMonth / 1000)} тыс ₽/мес на карточку при ${o.items} конкурентах.` });
  }
