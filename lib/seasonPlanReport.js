...
export async function collectFromCategory({
  ...
  // Приоритет: релевантность (совпадения слов), затем выручка.
  items.sort((a, b) => (b._relevance || 0) - (a._relevance || 0) || b.revenue - a.revenue);
...
export async function buildSeasonPlanReport({
  ...
  const baseDaily = (baseDailyOverride != null ? baseDailyOverride : shapeBase.base) * ambition;
  ...
  if (forecast && forecast.from && forecast.to && groupDaily.length > 0) {
    ...
    const fc = buildForecast({
      ...
      opts: { ...plan, baseSource: baseInfo.source },
    });
