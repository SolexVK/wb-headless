...
export class WbClient {
  ...
  constructor({ token, tokenType, baseUrls = {}, maxRetries = 3, timeoutMs = 30000 } = {}) {
    // Токен: явный аргумент → WB_API_TOKEN → Wildberries_API (имя из .env/секретов проекта).
    this.token = token || process.env.WB_API_TOKEN || process.env.Wildberries_API;
    this.tokenType = (tokenType || process.env.WB_TOKEN_TYPE || 'base').toLowerCase();
    if (!TOKEN_LIMITS[this.tokenType]) this.tokenType = 'base';
  ...
  async request(category, pathAndMaybeQuery, opts = {}) {
    if (!this.token) throw new Error('WB_API_TOKEN не задан');
