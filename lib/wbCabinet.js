...
// Origin, к которому привязан localStorage кабинета (там лежит токен WB Passport).
const CABINET_ORIGIN = process.env.WB_CABINET_ORIGIN || 'https://seller.wildberries.ru';

/**
 * Парсит localStorage из текста «name value» (по одной записи в строке; value —
 * всё после первого пробела) ИЛИ из JSON ([{name,value}] / {name:value}).
 * @returns {Array<{name:string,value:string}>}
 */
export function parseLocalStorage(raw) {
  const t = String(raw || '').trim();
  if (!t) return [];
  if (t.startsWith('[') || t.startsWith('{')) {
    const j = JSON.parse(t);
    if (Array.isArray(j)) return j.map((e) => ({ name: e.name, value: String(e.value) }));
    return Object.entries(j).map(([name, value]) => ({ name, value: String(value) }));
  }
  const out = [];
  for (const line of t.split(/\r?\n/)) {
    ...
    if (!s) continue;
    const i = s.search(/\s/);
    if (i === -1) continue; // строка без значения — пропускаем
    out.push({ name: s.slice(0, i), value: s.slice(i + 1).trim() });
  }
  return out.filter((e) => e.name && e.value);
}

/**
 * localStorage из первого доступного источника: аргумент → env WB_LOCALSTORAGE →
 * файл .secrets/wb-localstorage.txt. Пусто — это ок (не у всех страниц нужен).
 * @returns {{items: Array<{name,value}>, source: string}}
 */
export function resolveLocalStorage(opts = {}) {
  if (opts.localStorage) return { items: parseLocalStorage(
    typeof opts.localStorage === 'string' ? opts.localStorage : JSON.stringify(opts.localStorage)
  ), source: 'аргумент' };
  const env = process.env.WB_LOCALSTORAGE;
  if (env && env.trim()) return { items: parseLocalStorage(env), source: 'env:WB_LOCALSTORAGE' };
  const p = opts.localStorageFile || process.env.WB_LOCALSTORAGE_FILE ||
    path.join(REPO_ROOT, '.secrets', 'wb-localstorage.txt');
  if (fs.existsSync(p)) return { items: parseLocalStorage(fs.readFileSync(p, 'utf8')), source: `file:${p}` };
  return { items: [], source: 'нет' };
}
...
export async function launchCabinet(opts = {}) {
  ...
  const ls = resolveLocalStorage(opts);

  const proxyArgs = proxy ? ['--ssl-version-max=tls1.2', '--ignore-certificate-errors'] : [];
  log(`[cabinet] chromium: ${resolveBrowser() || '(дефолт PW)'} | прокси: ${proxy || 'нет'} | ` +
      `кук: ${cookies.length} (${source}) | localStorage: ${ls.items.length} (${ls.source})`);
  ...
  // storageState = куки + localStorage нужного origin (там лежит токен WB Passport).
  const storageState = {
    cookies,
    origins: ls.items.length ? [{ origin: CABINET_ORIGIN, localStorage: ls.items }] : [],
  };
