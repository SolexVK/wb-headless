...
export async function launchCabinet(opts = {}) {
  ...
  const cookies = opts.cookies
    ? normalizeCookies(opts.cookies)
    : loadCookiesFile(opts.cookiesFile).cookies;

  const proxyArgs = proxy ? ['--ssl-version-max=tls1.2', '--ignore-certificate-errors'] : [];
  log(`[cabinet] chromium: ${resolveBrowser() || '(дефолт PW)'} | прокси: ${proxy || 'нет'} | кук: ${cookies.length}`);
