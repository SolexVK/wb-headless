...
export const TARGET = {
  ...
  attributes: {
    fabric: {
      ...
      partial: {
        translucent_cotton: 0.8, // хлопок + «полупрозрачная» — это и есть марлевка
        linen_like: 0.5, // лён визуально близок: лёгкий, матовый, фактурный
        cotton_plain: 0.25, // просто хлопок без уточнений — не наша ткань
        smooth_matte: 0.3,
        unknown: null,
      },
      penalty: {}, // ткани нет в negative_criteria: хватает потери 20 баллов
      note: 'главный различитель ниши; берётся из «Состав» + «Фактура материала»',
    },
    silhouette: {
      weight: 14,
      target: 'oversize',
  ...
  // Признаки, без которых нельзя объявлять «очень близкий аналог».
  // Политика redistribute убирает вес невидимого признака из знаменателя —
  // для мелочей это правильно, но для главного различителя катастрофа:
  // карточка с неизвестной тканью набирала 100 «за всё остальное».
  // Здесь балл ограничивается сверху, а причина попадает в отчёт.
  capBandOnUnknown: { attrs: ['fabric'], maxScore: 89 },
...
export const FROM_TEXT = {
  fabric: (t) => {
    // Марлевка, муслин и жатка названы прямым текстом — самый сильный сигнал.
    if (t.gauzeClaim === true) return 'crinkled_gauze';
    const c = t.fabricClaim || '';
    if (/атлас|сатин|шелк|шёлк/i.test(c)) return 'satin_shiny';
    if (/флис|фланел|байк/i.test(c)) return 'flannel';
    if (/трикотаж|вязан/i.test(c)) return 'knit';
    if (/джинс|деним/i.test(c)) return 'denim';
    if (/лен|лён|linen/i.test(c)) return 'linen_like';
    // Хлопок с полупрозрачной фактурой — это марлевка, просто продавец
    // не назвал её по имени. Проверено на целевых карточках ниши.
    if (/хлоп|cotton/i.test(c) && t.translucent === true) return 'translucent_cotton';
    if (/хлоп|cotton/i.test(c)) return 'cotton_plain';
    return null;
  },
