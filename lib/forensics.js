...
export function analyzeLogistics(src) {
  ...
  const pendingAll = issued.filter(
    (r) => !chargedSrids.has(low(r.srid)) && !chargedShk.has(String(r.srid))
  );
  // ПВЗ, где тарификации не было ни разу, из прогноза исключаем.
  const freePvz = new Set(pvzStats.filter((p) => p.charged === 0).map((p) => p.pvz));
  const pending = pendingAll.filter((r) => !freePvz.has(r.dstOfficeAddress || '—'));
  ...
  return {
    ...
    pvzStats,
    pending: { count: pending.length, forecast: round(pending.length * avgTariff), freePvz: [...freePvz] },
...
export function analyzeFines(src) {
  ...
  const ageDays = (r) => (r.readyToReturnDt ? (Date.now() - new Date(r.readyToReturnDt)) / DAYS : 0);
