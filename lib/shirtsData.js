...
async function toDataUri(url) {
  ...
  for (let i = 0; i < 3; i++) {
    try { const r = await fetch(abs); if (r.ok) { const b = Buffer.from(await r.arrayBuffer()); if (b.length) return `data:${r.headers.get('content-type') || 'image/webp'};base64,${b.toString('base64')}`; } else break; }
    catch (_) { /* retry */ }
...
export async function buildShirtsData({ skus, d1, d2, categoryPath = 'Мужчинам/Рубашки', sizesQuery = 'рубашка мужская с длинным рукавом', onProgress } = {}) {
  ...
  for (let i = 0; i < skus.length; i++) {
    ...
    const img = await toDataUri(photoF);
