...
function svgChart(daily, opts) {
  ...
  const round100 = secondaryUnit === '₽' ? 100 : 1;
  const minS = secondaryUnit === '₽' ? Math.floor(minSraw / round100) * round100 : 0;
  const maxS = Math.ceil(maxSraw / round100) * round100 || 1;
...
<text x="${padL + plotW + 6}" y="${(yy + 3).toFixed(1)}" font-size="9" fill="${secondaryColor}">${fmt(pvp)}</text>`);
  ...
  const legend = `<g font-size="10">
    ...
    <text x="${padL + 23}" y="${padT - 5}" fill="var(--fg)">${primaryLabel}, ${primaryUnit} (левая ось)${smoothPrimary ? ', сглажено' : ''}</text>
    <line x1="${padL + 250}" y1="${padT - 8}" x2="${padL + 268}" y2="${padT - 8}" stroke="${secondaryColor}" stroke-width="1.8" stroke-dasharray="5 3"/>
    <text x="${padL + 273}" y="${padT - 5}" fill="var(--fg)">${secondaryLabel}, ${secondaryUnit} (правая ось)</text>
  </g>`;
...
<div class="chart">${svgChart(p.forecastDaily || [], { primaryKey: 'plannedOrders', primaryLabel: 'план заказов', primaryColor: '#2646A0', primaryUnit: 'шт', secondaryKey: 'price', secondaryLabel: 'цена', secondaryColor: '#C1121F', secondaryUnit: '₽', smoothPrimary: true })}</div>
<h2>История спроса за 2 года (реальные данные аналогов)</h2>
<div class="chart">${svgChart(p.historyDaily || [], { primaryKey: 'sales', primaryLabel: 'продажи аналогов', primaryColor: '#2E7D5B', primaryUnit: 'шт', secondaryKey: 'price', secondaryLabel: 'цена', secondaryColor: '#C1121F', secondaryUnit: '₽', smoothPrimary: false })}</div>
<h2>Остатки конкурентов vs продажи (где остаток ниже спроса — благоприятно)</h2>
<div class="chart">${svgChart(p.historyDaily || [], { primaryKey: 'stock', primaryLabel: 'остатки конкурентов', primaryColor: '#8E7CC3', primaryUnit: 'шт', secondaryKey: 'sales', secondaryLabel: 'продажи', secondaryColor: '#2E7D5B', secondaryUnit: 'шт/день', smoothSecondary: true })}</div>`
    : `<div class="chart">${svgChart(p.daily || [], { primaryKey: 'plannedOrders', primaryLabel: 'план заказов', primaryColor: '#2646A0', primaryUnit: 'шт', secondaryKey: 'price', secondaryLabel: 'цена', secondaryColor: '#C1121F', secondaryUnit: '₽', smoothPrimary: false })}</div>`;
