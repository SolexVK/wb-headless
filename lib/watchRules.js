 ...
 */
export function buildFindings({ rows = [], baselineRows = [], returns = [], orders = [], fbsOrders = [], warehouses = [], thresholds = {}, state = {} } = {}) {
  const T = {
  ...
  const src = { finance: rows, returns, orders, fbsOrders, warehouses, sales: [] };
  ...
  if (aw.overdue > 0) {
    const key = `returns-overdue|${new Date().toISOString().slice(0, 10)}`;
    findings.push({
      key, severity: 'medium', catalogId: 'pvz-return-storage',
      title: 'Возвраты в платной зоне хранения', amount: round(aw.overdue * (fines.storage.dailyRate || 0)), count: aw.overdue,
      ...
      facts: [
        ...
        `Ставка ${fmt(fines.storage.dailyRate || 0, 2)} в сутки → ≈${fmt(aw.overdue * (fines.storage.dailyRate || 0))} в день`,
  ...
  if (log.pending.forecast >= T.pendingLogisticsAlert) {
    const key = `pending-logistics|${new Date().toISOString().slice(0, 10)}`;
    findings.push({
      ...
      facts: [
        ...
        log.pending.freePvz.length ? `Не тарифицируются: ${log.pending.freePvz.map((p) => String(p).split(',')[0]).join(', ')}` : null,
      ].filter(Boolean),
      actions: ['Заложить сумму в план недели', 'Проверить, можно ли перевести возвратный поток на ПВЗ без тарификации'],
      isNew: !seen[key],
    });
  }
