...
<section class="page-break">
  ...
  ${freePvz.length ? `
  <div class="banner finding">
    <b>Находка: возвраты через ${esc(freePvz[0].pvz)} не тарифицируются вовсе</b>
    <p>За наблюдаемый период по этому ПВЗ выдано <b>${fmt(freePvz.reduce((s, p) => s + p.issued, 0))}</b> возвратов и <b>ни одного</b> начисления логистики,
    тогда как ${paidPvz.map((p) => esc(p.pvz.split(',')[0])).join(' и ')} тарифицируются регулярно.
    Это самый крупный рычаг экономии: при среднем тарифе ${fmt(log.avgTariff)} сом перевод возвратного потока на этот ПВЗ
    экономит порядка <b>${fmt(freePvz.reduce((s, p) => s + p.issued, 0) * log.avgTariff)} сом</b> на таком же объёме.
    Проверьте на следующем отчёте (28.08 ещё не закрыт) — если начислений так и не появится, гипотеза подтверждена.</p>
  </div>` : ''}
...
<section>
  ...
  <div class="cols">
    ...
    <div class="card"><b>«Удерживают не за все поставки»</b>
      ...
      <li>Исключение — «бесплатный» ПВЗ из раздела 4.</li></ul></div>
  </div>
</section>
...
<section>
  ...
  <div class="banner warn">
    ...
    <p>1. <b>Сменить ПВЗ возвратов</b> на тот, где тарификации нет (раздел 4) — потенциально до ${fmt(log.total)} сом за такой же период.<br>
    2. <b>Снизить сам поток возвратов</b>: ${fmt(log.rowCount)} перевозок — это ${fmt(log.rowCount)} невыкупов и браков. Топ-артикулы в разделе 7:
    у лидеров проблема системная (размерная сетка, фото, описание, качество пошива).<br>
    3. <b>Уходить со складов с коэффициентом ×2</b>: средний чек перевозки там ${fmt((log.byCoef.find((c) => Number(c.key) === 2) || { avg: 0 }).avg, 2)} сом против
    ${fmt((log.byCoef.find((c) => Number(c.key) === 1) || { avg: 0 }).avg, 2)} сом при ×1.<br>
    4. <b>Забирать возвраты вовремя</b> — иначе к логистике добавляется штраф за хранение на ПВЗ (см. второй отчёт).</p>
  </div>
</section>
