...
export async function buildStockAvailabilityReport({
  ...
  concurrency = 5,
  onProgress,
  dataSource, // необяз.: bulk-источник (items,d1,d2)→Map<sku,daily[]>. Если задан,
              //          данные тянутся одним пакетом (напр. WB API), а не по SKU.
} = {}) {
  ...
  // ── Ветка bulk-источника (WB API): один пакетный запрос на весь список ──
  // Так мы не долбим API по одному SKU и не рискуем каскадом 429.
  if (typeof dataSource === 'function') {
    const seriesMap = await dataSource(unique, d1, d2);
    const rows = unique.map((item) => {
      const sku = item.wb;
      const daily = seriesMap.get(String(sku)) || [];
      const m = computeSkuMetrics(sku, daily);
      if (onProgress) onProgress(++done, unique.length, sku);
      return { seller: item.seller, ...m };
    });
    return assembleReport({ rows, unique, errors, dailyLimit: null, d1, d2 });
  }
  ...
  return assembleReport({ rows, unique, errors, dailyLimit, d1, d2 });
}

/**
 * Общая сборка отчёта из посчитанных строк: сортировка по упущенной выручке,
 * итоги, служебные счётчики. Используется обеими ветками (MPSTATS и WB API).
 */
function assembleReport({ rows, unique, errors, dailyLimit, d1, d2 }) {
