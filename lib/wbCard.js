...
/**
 * GET с ретраями. 404 возвращаем сразу — это законный ответ «здесь нет».
 * Сетевые сбои, тайм-ауты, 429 и 5xx повторяем: за десятичасовой прогон
 * одна осечка иначе потеряет карточку навсегда.
 */
async function httpGet(url, { timeoutMs = 8000, retries = 3 } = {}) {
  for (let attempt = 0; ; attempt += 1) {
    ...
    await sleep(400 * 2 ** attempt); // 0.4с, 0.8с, 1.6с
  }
}

/** Проба шарда: 404 — не этот шард, сеть — повторяем один раз. */
async function probe(url) {
  const r = await httpGet(url, { timeoutMs: 6000, retries: 1 });
...
export async function resolveShard(nm) {
  ...
  if (cache[vol]) {
    ...
    if (await probe(`${CDN_HOST(cache[vol])}${basePath(nm)}/info/ru/card.json`)) return cache[vol];
  }
  for (let i = 1; i <= MAX_SHARD; i += 1) {
    if (await probe(`${CDN_HOST(i)}${basePath(nm)}/info/ru/card.json`)) {
      cache[vol] = i;
      saveCache();
      return i;
    }
...
export async function fetchCard(nm) {
  ...
  const r = await httpGet(url, { timeoutMs: 15000, retries: 3 });
