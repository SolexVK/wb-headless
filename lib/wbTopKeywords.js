...
import { enrichRows, openCardCache } from './wbCard.js';
import { applyFacets } from './wbFacets.js';
...
export function normalizeSearchRow(row = {}, idx = 0) {
  ...
  // Порядок полей: сначала артикул → бренд → название → цвет/фото, затем метрики.
  return {
    nmId: nmRaw == null ? null : String(nmRaw),
    brand: row.brand ?? row.brand_name ?? null,
    name: row.name ?? row.title ?? null,
    color: row.color ?? row.colour ?? null,
    thumb: row.thumb ?? row.thumb_middle ?? row.image ?? null, // первое фото карточки
...
// Текст для матчинга: название + характеристики + описание карточки (если карточка
// обогащена через enrichRows) — иначе одно название. Признак «оверсайз» в характеристиках
// ловится, даже если его нет в названии.
const matchTextOf = (r) => r._matchText || lc(r.name);
...
export function selectTop(rows = [], opts = {}) {
  const {
    ...
    minSales,
    excludeNmIds = [],
    facets = [], // активные фасеты-признаки (buildFacetFilters): плюс/минус стемы
    topN,
  } = opts;
  ...
  const valid = rows.filter((r) => r && r.nmId != null && !exNm.has(r.nmId));
  const pool = valid.filter((r) => r.revenue >= revenueFloor);

  // Воронка отсева — считаем, сколько где отсеялось (для прозрачности пользователю).
  const funnel = {
    fetched: rows.length,
    belowRevenue: valid.length - pool.length, // < revenueFloor
    byMetrics: 0, // цена/рейтинг/отзывы/продажи
    byExclude: 0, // слова-исключения
    byFacet: {}, // {label: count} — по каждому признаку
    byGroup: 0, // не совпал ни с одной группой и не «сильный»
    kept: 0,
    shown: 0,
  };
  const bumpFacet = (label) => {
    funnel.byFacet[label] = (funnel.byFacet[label] || 0) + 1;
  };
  ...
  const poolForThreshold = exclude.length ? pool.filter((r) => !hasAny(matchTextOf(r), exclude)) : pool;
  ...
  const items = [];
  let uncertainPattern = 0;
  for (const r of pool) {
    if (!passMetrics(r)) {
      funnel.byMetrics++;
      continue;
    }
    const text = matchTextOf(r); // название + характеристики + описание (если обогащено)

    // Слово из общего exclude в тексте карточки — выкидываем всегда.
    if (exclude.length && hasAny(text, exclude)) {
      funnel.byExclude++;
      continue;
    }

    // Фильтр по признакам-фасетам: выбранное значение = плюс-ключ, остальные
    // значения признака = минус-ключи. drop — конфликт; uncertain — сигнала нет
    // (🟡, оставляем с пометкой — кандидат на ручную/vision проверку).
    let uncFacets = [];
    if (facets.length) {
      const { drop, dropLabel, uncertain } = applyFacets(text, facets);
      if (drop) {
        bumpFacet(dropLabel);
        continue;
      }
      uncFacets = uncertain;
      if (uncertain.length) uncertainPattern++;
    }

    // Публичная строка контракта: без внутренних полей обогащения.
    const { _card, _matchText, _pattern, ...pub } = r;
    if (uncFacets.length) {
      pub.uncertainFacets = uncFacets; // по каким признакам сигнала нет
      pub.patternUncertain = true; // общий флаг 🟡 «нужна визуальная проверка»
    }

    // Совпадение хотя бы по одной группе (ИЛИ). Групп нет — проходят все.
    const groupMatch = groups.length === 0 || groups.some((g) => hasAny(text, g.any || []));
    if (groupMatch) {
      items.push({ ...pub, matchType: groups.length ? 'group' : 'all' });
      continue;
    }

    // Иначе — «безхвостый» (ключей групп в тексте нет): оставляем, если он сильный.
    if (r.revenue >= exceptionRevenueThreshold) {
      items.push({ ...pub, matchType: 'exception' });
    } else {
      funnel.byGroup++;
    }
  }
  ...
  funnel.kept = items.length;
  funnel.shown = cut.length;
  return { pool: pool.length, exceptionRevenueThreshold, items: cut, uncertainPattern, funnel };
}
...
export async function topByKeywords(p = {}) {
  const { query, d1, d2, filters = {}, our, topN, maxRows, searchCache } = p;
  if (!query || !String(query).trim()) {
    throw new Error('topByKeywords: не задана ключевая фраза (query)');
  }

  // Кэш выдачи MPStats (Слой 1): ключ (фраза, период). Историч. период неизменен,
  // поэтому попадание экономит запрос к MPStats (дневной лимит). searchCache —
  // необязательный интерфейс {get(key), set(key,val)} от вызывающего (CLI/бот).
  const phrase = String(query).trim();
  const cacheKey = { phrase, d1: d1 || null, d2: d2 || null, maxRows: maxRows || null };
  let search = searchCache ? searchCache.get(cacheKey) : null;
  let fromCache = !!search;
  if (!search) {
    search = await fetchSearchResults(phrase, { d1, d2, maxRows });
    if (searchCache) searchCache.set(cacheKey, search);
  }
  const { rows: raw, total, period, capped } = search;
  ...
  // Обогащение карточками (характеристики+описание) нужно, только если есть фильтры
  // по словам/рисунку — иначе матчим по названию и лишних запросов к CDN не делаем.
  const deep = filters.deep || {};
  const needEnrich =
    !p.noEnrich &&
    ((deep.exclude && deep.exclude.length) ||
      (deep.groups && deep.groups.length) ||
      (Array.isArray(filters.facets) && filters.facets.length));
  let enrichStat = null;
  if (needEnrich) {
    const floor = filters.revenueFloor ?? REVENUE_FLOOR_DEFAULT;
    const exNm = new Set(excludeNmIds.map(String));
    const toEnrich = normalized.filter((r) => !exNm.has(r.nmId) && r.revenue >= floor);
    const cache = openCardCache();
    enrichStat = await enrichRows(toEnrich, { cache });
  }

  const { pool, exceptionRevenueThreshold, items, uncertainPattern, funnel } = selectTop(normalized, {
    ...filters,
    excludeNmIds,
    topN,
  });
  ...
  return {
    ...
    enrich: enrichStat, // статистика добора карточек (или null)
    searchFromCache: fromCache, // выдача взята из кэша (лимит MPStats не потрачен)
    uncertainPattern: uncertainPattern || 0, // сколько 🟡 «признак не определён»
    funnel, // воронка отсева: где и сколько отсеялось
    rivals: items,
    // вся база из MPStats (до deep-фильтров) — для блока «Доминирующие цвета» [3]:
    // цвет/заказы/выручка по каждому артикулу всей выдачи.
    allItems: normalized.map((r) => ({
      nmId: r.nmId, brand: r.brand, name: r.name, color: r.color,
      revenue: r.revenue, sales: r.sales, avgSalePrice: r.avgSalePrice,
    })),
  };
}

/**
 * Скачивает первое фото каждого артикула и вшивает его в объект как data-URI
 * (`r.thumbData`) — чтобы HTML-отчёт был самодостаточным и картинки показывались
 * даже там, где внешние хосты заблокированы. Мутирует переданные объекты.
 * Данные MPStats уже содержат ссылку `thumb` — доп. запросов к API нет, только
 * скачивание крошечных webp (~12 КБ) с CDN Wildberries. Ошибки — молча пропускаем.
 * @param {object[]} rivals  элементы результата (с полем thumb)
 * @param {object} o  { concurrency=8, timeoutMs=8000 }
 */
export async function embedThumbnails(rivals = [], { concurrency = 8, timeoutMs = 8000 } = {}) {
  const queue = rivals.filter((r) => r && r.thumb && !r.thumbData);
  let idx = 0;
  const worker = async () => {
    while (idx < queue.length) {
      const r = queue[idx++];
      try {
        const resp = await fetch(r.thumb, { signal: AbortSignal.timeout(timeoutMs) });
        if (!resp.ok) continue;
        const ct = resp.headers.get('content-type') || 'image/webp';
        const buf = Buffer.from(await resp.arrayBuffer());
        r.thumbData = `data:${ct};base64,${buf.toString('base64')}`;
      } catch (_) { /* фото недоступно — пропускаем */ }
    }
  };
...
// ── Таблица-пикер для терминала (шаг выбора конкурентов [1]→[2]) ───────────────
// Печатает нумерованную моноширинную таблицу, из которой человек называет номера
// строк или прямо nmId — выбранные артикулы уходят конкурентами в «Сравнение
// карточек» ([2], scripts/wb-cards-compare.mjs). Строго stdout, без сети.
const PICK_MATCH = { group: 'совп', exception: 'безхв', all: '—' };

// Обрезка строки до ширины с многоточием (по видимым символам, не байтам).
const truncate = (s, w) => {
  const str = String(s ?? '—');
  return [...str].length <= w ? str : [...str].slice(0, w - 1).join('') + '…';
};
const padR = (s, w) => { const str = String(s ?? ''); const gap = w - [...str].length; return gap > 0 ? str + ' '.repeat(gap) : str; };
const padL = (s, w) => { const str = String(s ?? ''); const gap = w - [...str].length; return gap > 0 ? ' '.repeat(gap) + str : str; };

/**
 * Нумерованная таблица-пикер результата [1] для терминала.
 * Колонки: #, nmId, бренд, название (усечено), ср.цена, выручка, тип.
 * Номер строки (#) совпадает с порядком в res.rivals и в HTML-отчёте — по нему
 * пользователь называет конкурентов для [2].
 * @param {object} res  контракт top-rivals (результат topByKeywords)
 * @param {object} o  { nameWidth=42, brandWidth=16 }
 * @returns {string} готовый к печати текст (без завершающего перевода строки)
 */
export function formatPickTable(res, { nameWidth = 42, brandWidth = 16 } = {}) {
  const rivals = res?.rivals || [];
  if (!rivals.length) return 'Пусто: под фильтры не попал ни один артикул.';

  const numW = Math.max(1, String(rivals.length).length);
  const nmW = Math.max(6, ...rivals.map((r) => String(r.nmId ?? '').length));
  const header =
    padL('#', numW) + '  ' + padR('nmId', nmW) + '  ' + padR('бренд', brandWidth) +
    '  ' + padR('название', nameWidth) + '  ' + padL('ср.цена', 9) + '  ' + padL('выручка ₽', 12) + '  тип';
  const sep = '─'.repeat([...header].length);

  const lines = rivals.map((r, i) =>
    padL(i + 1, numW) + '  ' +
    padR(r.nmId ?? '—', nmW) + '  ' +
    padR(truncate(r.brand || '—', brandWidth), brandWidth) + '  ' +
    padR(truncate(r.name || '—', nameWidth), nameWidth) + '  ' +
    padL(rub(r.avgSalePrice), 9) + '  ' +
    padL(rub(r.revenue), 12) + '  ' +
    (PICK_MATCH[r.matchType] || '—'));

  const hint =
    `\nВыбор конкурентов для «Сравнения карточек» [2]: назови НОМЕРА строк (напр. 1,4,7)\n` +
    `или прямо nmId. Затем: node scripts/wb-cards-compare.mjs --our <НАШ> --rivals <nmId,…>`;
...
export function formatHtml(res) {
  ...
  const rows = res.rivals.map((r, i) => {
    ...
    const imgSrc = r.thumbData || r.thumb || '';
    const img = imgSrc
      ? `<a href="${url}" target="_blank" rel="noopener"><img class="thumb" src="${imgSrc}" alt="" loading="lazy"></a>`
...
<style>
  ...
  td.pic { width:56px; padding:4px; }
  img.thumb { width:48px; height:64px; object-fit:cover; border-radius:5px; display:block; background:#eee; }
  .noimg { color:#bbb; }
...
<table>
  <thead><tr>
    <th>#</th><th>Фото</th><th>Артикул</th><th>Бренд</th><th>Название</th><th>Цвет</th><th>Тип</th>
