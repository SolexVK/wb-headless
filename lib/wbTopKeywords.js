...
import { enrichRows, openCardCache, patternVerdict } from './wbCard.js';
...
export function selectTop(rows = [], opts = {}) {
  const {
    ...
    minSales,
    excludeNmIds = [],
    pattern, // желаемый рисунок ткани: однотон/полоска/клетка/… ('' — любой)
    topN,
  } = opts;
  ...
  for (const r of pool) {
    ...
    // Фильтр по рисунку ткани (если задан): drop — конфликтующий/иной рисунок,
    // uncertain — сигнала нет (🟡, оставляем с пометкой — кандидат на vision/ручной).
    let patternFlag = null;
    if (pattern) {
      const verdict = patternVerdict(r._pattern || new Set(), pattern);
    ...
    // Публичная строка контракта: без внутренних полей обогащения (_card/_matchText/
    // _pattern). Для прозрачности оставляем компактные patternTags (если считались).
    const { _card, _matchText, _pattern, ...pub } = r;
    if (pattern && _pattern) pub.patternTags = [..._pattern];
    if (patternFlag) pub.patternUncertain = true;
...
export async function topByKeywords(p = {}) {
  ...
  const needEnrich =
    !p.noEnrich &&
    ((deep.exclude && deep.exclude.length) || (deep.groups && deep.groups.length) || !!filters.pattern);
  let enrichStat = null;
  if (needEnrich) {
    ...
    enrichStat = await enrichRows(toEnrich, { cache, desiredPattern: filters.pattern });
  }
