...
export function selectTop(rows = [], opts = {}) {
  ...
  for (const r of pool) {
    ...
    if (exclude.length && hasAny(text, exclude)) continue;
    ...
    let uncFacets = [];
    if (facets.length) {
      const { drop, uncertain } = applyFacets(text, facets);
      if (drop) continue;
      uncFacets = uncertain;
  ...
  return { pool: pool.length, exceptionRevenueThreshold, items: cut, uncertainPattern };
}
...
export async function topByKeywords(p = {}) {
  ...
  const { pool, exceptionRevenueThreshold, items, uncertainPattern } = selectTop(normalized, {
    ...filters,
    excludeNmIds,
    topN,
  });
  ...
  return {
    ...
    uncertainPattern: uncertainPattern || 0, // сколько 🟡 «рисунок не определён»
    rivals: items,
