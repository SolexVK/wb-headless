...
/**
 * Полная карточка товара по SKU за период [d1, d2]: бренд, фото, ссылка,
 * остатки и агрегированная статистика периода (period_stats). Сырой объект
 * MPSTATS — интерпретируется в вызывающем коде (структура зависит от версии).
 * @returns {Promise<object|null>}
 */
export async function fetchItemFull(sku, d1, d2) {
  const path = `/wb/get/item/${encodeURIComponent(sku)}/full?d1=${d1}&d2=${d2}`;
  return apiGet(path);
}
...
function normalizeCategoryRow(row) {
  ...
  return {
    ...
    revenueGraph: Array.isArray(row.revenue_graph) ? row.revenue_graph.map((v) => Number(v) || 0) : null,
    priceGraph: Array.isArray(row.price_graph) ? row.price_graph.map((v) => Number(v) || 0) : null,
    // фото и ссылка на карточку (для товарных отчётов)
    thumb: str(row.thumb),
    thumbMiddle: str(row.thumb_middle ?? row.thumb),
    url: str(row.url) || (row.id != null ? `https://www.wildberries.ru/catalog/${row.id}/detail.aspx` : ''),
    color: str(row.color),
    // остатки по типу хранения и складам
    balanceFbs: num(row.balance_fbs),
    warehousesCount: num(row.warehouses_count),
    frozenPct: num(row.frozen_stocks_percent),
    // реклама
