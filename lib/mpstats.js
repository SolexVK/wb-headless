 ...
 */
async function apiGet(pathAndQuery, { retries = 3 } = {}) {
  ...
  while (attempt <= retries) {
    try {
      ...
      if (!resp.ok) {
        ...
        const err = new Error(`MPSTATS ${resp.status}: ${body.slice(0, 200)}`);
        // Клиентские ошибки (401/403/400 и т.п.) повтором не лечатся —
        // падаем сразу и показываем текст. Повторяем только 429 и 5xx.
