...
// --- Заключение: буллеты с цветными маркерами ---
function conclusionBox(c) {
  ...
  return `<div class="concl" style="--vc:${vc}">
    <div class="chead"><span class="cflag" style="background:${vc}">${esc(c.verdict)}</span>
      <span class="cscore">${c.total}/100</span><span class="cbn">узкое место: ${esc(c.bottleneck)}</span></div>
...
export function buildNicheHtml(a) {
  ...
  <section class="grid2">
    <div class="card">
      <div class="ctitle">Sell-through — продажи ÷ остатки (метод Овчинникова)</div>
      ${sellThroughBar(sat)}
    ...
    <div class="card">
      <div class="ctitle">Распределение выручки по цене</div>
  ...
  ${qShownP.length ? `<section class="card">
    <div class="ctitle">Уточняющие фразы — sell-through и спрос ${qAll.length > qShownP.length ? `<span class="cap">(топ ${qShownP.length} из ${qAll.length})</span>` : ''}</div>
    <table class="phrases">
      <thead><tr><th>Запрос</th><th class="num">Sell-through</th><th class="num">Оборот</th><th class="num">Спрос:предл.</th><th class="num">Частота</th></tr></thead>
      <tbody>${phraseRows}</tbody>
    </table>
    <div class="cap">Sell-through = продажи÷остаток по ТОП-выдаче (>1.5× — дефицит, выгодно). Оборот — медиана дней распродажи. Спрос:предл. и частота — из Оракула (если заданы).</div>
  </section>` : ''}
