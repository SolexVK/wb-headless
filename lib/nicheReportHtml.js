...
// Цвет по тексту вердикта (учитывает понижение жёсткими стопами, а не только балл).
const verdictTextColor = (v) => (v === 'Заходить' ? C.good : v === 'Не заходить' ? C.bad : C.mid);
...
function conclusionBox(c) {
  if (!c) return '';
  const vc = verdictTextColor(c.verdict) || verdictColor(c.total);
...
export function buildNicheHtml(a) {
  ...
  const vc = verdictTextColor(sc.verdict);
