...
function scoreBarsSvg(blocks, real) {
  ...
  const W = 524, rowH = 40, barX = 214, barW = 246, top = 4;
  ...
  rows.forEach(([title, b, val], i) => {
    ...
    // левая колонка: название + реальное значение
    s += `<text x="0" y="${cy - 2}" font-size="12.5" font-weight="700" fill="${C.ink}">${esc(title)}</text>`;
    s += `<text x="0" y="${cy + 12}" font-size="9.5" fill="${C.muted}">${esc(val || '')}</text>`;
    // бар
    const by = cy - 9;
...
function sellThroughTimelineSvg(tl) {
  ...
  list.forEach((m, i) => {
    ...
    s += `<text x="${x + w / 2}" y="${H - 8}" font-size="8.5" fill="${m.favorable ? C.good : C.muted}" font-weight="${m.favorable ? 700 : 400}" text-anchor="middle">${esc(m.label)}</text>`;
  });
  return s + '</svg>';
}
...
function priceSegmentsSvg(segs) {
  ...
  const W = 520, H = 138, padT = 8, padB = 30, padL = 8;
  ...
  list.forEach((seg, i) => {
    ...
    if (i % Math.ceil(list.length / 8) === 0 || isTop) {
      s += `<text x="${x + w / 2}" y="${H - 18}" font-size="8" fill="${isTop ? C.accent : C.muted}" text-anchor="middle">${int(seg.minPrice)}</text>`;
    }
  });
  s += `<text x="${padL}" y="${H - 5}" font-size="9" fill="${C.muted}">Выручка по цене (₽): пик — <tspan fill="${C.accent}" font-weight="700">${int(topSeg.minPrice)}–${int(topSeg.maxPrice)} ₽</tspan></text>`;
  return s + '</svg>';
}
...
export function buildNicheHtml(a) {
  ...
  // Перспективные сегменты — минимум два.
  const opps = (a.priceOpportunity?.opportunities || []).slice(0, 2);
  const oppRows = opps.map((o, i) => `<div class="opp">
      <span class="oppn">${i + 1}</span>
      <b style="color:${C.good}">${int(o.from)}–${int(o.to)} ₽</b> — ${money(o.revenuePerCardMonth)}/мес на карточку ·
    ...
    <div class="card">
      ...
      ${ec ? economicsSvg(ec) +
        ...
        (ec.target && ec.target.competitors
          ? `<div class="etar">При цене <b>${int(ec.target.price)} ₽</b> (целевая маржа 30%) — сегмент ${int(ec.target.segFrom)}–${int(ec.target.segTo)} ₽, ~${int(ec.target.competitors)} конкурентов:<br>
              продажи ~<b style="color:${C.accent}">${int(ec.target.salesPerCardMonth)} шт/мес</b> на карточку · выручка ~<b>${money(ec.target.revenuePerCardMonth)}/мес</b> · прибыль ~<b style="color:${C.good}">${money(ec.target.profitPerCardMonth)}/мес</b>.</div>`
  ...
  <section class="grid2">
    <div class="card">
      ...
      ${oppRows ? `<div class="opps">${oppRows}</div><div class="cap">Меньше конкуренции при высокой выручке на карточку — приоритет более дорогим сегментам (массовость не нужна).</div>` : ''}
    </div>
    <div class="card">
