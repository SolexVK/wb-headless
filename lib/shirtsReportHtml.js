  ...
  return `<article class="art">
    <div class="art-head">
      ...
      <div class="art-id">
        ...
        <div class="money3">
          <div class="mt mt-rev"><span class="mt-l">Выручка за год</span><span class="mt-v">${money(m.revenue)}</span><span class="mt-s">${int(m.units)} шт · ${int(m.avgPrice)} ₽</span></div>
...
<body class="${book ? 'book' : ''}"><div class="wrap">
  <div class="head">
    ...
    <h1>Мужские однотонные рубашки с длинным рукавом — рынок, топ и разбор 3 артикулов</h1>
    <div class="sub">${esc(market.categoryPath)} · период ${esc(d1)} — ${esc(d2)} (год) · артикулов в разборе: ${articles.length}<br>
      <span style="font-size:11px">Ниша: обычные однотонные рубашки с длинным рукавом (исключены короткий рукав, тёплые/фланель, клетка, полоска)</span></div>
  </div>
