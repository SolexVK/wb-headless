...
export function buildShirtsReportHtml(data) {
...
<style>
  * { box-sizing: border-box; }
  ...
  @media print {
    body { background: #fff; }
    .wrap { max-width: none; padding: 0 5mm; }
    .art, .nt-row, .card, .hkpi { break-inside: avoid; }
    .ph:hover img, .nt-ph img:hover { transform: none; box-shadow: none; }
  }
  @page { size: A4; margin: 9mm 5mm; }
</style></head>
<body><div class="wrap">
  <div class="head">
    <div class="brandline">Анализ ниши · Wildberries · MPStats + публичные данные WB</div>
    <h1>Мужские рубашки с длинным рукавом — рынок, топ и разбор 3 артикулов</h1>
    <div class="sub">${esc(market.categoryPath)} · период ${esc(d1)} — ${esc(d2)} (год) · артикулов в разборе: ${articles.length}</div>
  </div>
