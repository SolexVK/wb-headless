...
export function buildForecast({ history, recent60, prior60, baseDaily, forecastFrom, forecastTo, opts = {} }) {
  ...
  const meanPrice = folded.meanPrice;
...
/** Ранг по свёрнутому профилю (p90/p50 месячных индексов). */
function rankFrom(folded) {
  const vals = folded.months.map((m) => m.index).sort((a, b) => a - b);
  if (!vals.length) return { rank: 'нет данных', code: 'none', amplitude: null };
  const q = (p) => vals[Math.min(vals.length - 1, Math.round((vals.length - 1) * p))];
  const amplitude = q(0.5) > 0 ? round(q(0.9) / q(0.5), 3) : Infinity;
  const rank = amplitude >= 2 ? 'ярко выраженный' : amplitude >= 1.3 ? 'умеренный' : 'слабый';
  return { rank, code: amplitude >= 2 ? 'strong' : amplitude >= 1.3 ? 'moderate' : 'weak', amplitude, p90: q(0.9), p50: q(0.5) };
}
