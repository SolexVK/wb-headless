...
// Значения из текста, которые почти ничего не говорят. «Хлопок 70 %» —
// это и марлевка, и фланель, и поплин. Если зрение дало конкретный ответ,
// оно точнее: на плотной клетчатой карточке текст давал cotton_plain (0,25
// веса), а зрение — flannel (0), и право было зрение.
const WEAK_TEXT = {
  fabric: ['cotton_plain', 'smooth_matte'],
};

const isWeak = (attr, v) => Boolean(v && WEAK_TEXT[attr]?.includes(v));
...
export function mergeObservations(fromText = {}, fromVision = {}, profile = TARGET) {
  ...
  for (const attr of Object.keys(profile.attributes)) {
    ...
    // карточек, и продавец называет материал точнее, чем видно на фото.
    // Но только когда текст говорит что-то содержательное: расплывчатое
    // «хлопок» уступает конкретному ответу зрения.
    const textWins = (attr === 'fabric' || attr === 'silhouette')
      && !(isWeak(attr, textVal) && visVal);
...
export function scoreCard(values, profile = TARGET) {
  ...
  let score = failedGate ? 0 : Math.max(0, Math.round(base - penaltyTotal));

  // Потолок при невидимом ключевом признаке: «не знаем главного» не должно
  // читаться как «идеальное совпадение по всему остальному».
  const cap = profile.capBandOnUnknown;
  let cappedBy = null;
  if (!failedGate && cap) {
    const missing = cap.attrs.filter((a) => unknown.includes(a));
    if (missing.length && score > cap.maxScore) {
      cappedBy = missing;
      score = cap.maxScore;
    }
  }
...
export function explain(result, profile = TARGET) {
  ...
  if (result.cappedBy) {
    lines.push(`  ⌐ потолок 89: не виден ключевой признак — ${result.cappedBy.join(', ')}`);
  }
