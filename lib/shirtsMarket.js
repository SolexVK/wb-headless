...
async function searchJsonRetry(url, tries = 12) {
  for (let i = 0; i < tries; i++) {
    try { const r = await fetch(url); if (r.ok) return await r.json(); } catch (_) { /* ретрай */ }
    await new Promise((res) => setTimeout(res, Math.min(6000, 1500 * (i + 1)))); // до 6с между попытками — сброс 429
  }
  return null;
}
...
export async function buildShirtsMarket({ categoryPath, d1, d2, plus = '', minus = 'коротк', topN = 10, top300 = 300, sizesQuery, maxRows = 3000, nicheQueries = null, refine = null } = {}) {
  ...
  // Женская ниша: отсекаем мужские/детские карточки (по gender из выдачи поиска) и,
  // при необходимости, бренды-помехи (refine.excludeBrand — регэксп по бренду).
  const dropGenders = new Set(['Мужской', 'Мальчики', 'Девочки', 'Мальчикам', 'Девочкам']);
  const exBrand = refine?.excludeBrand ? new RegExp(refine.excludeBrand, 'i') : null;
  const filtered = items.filter((it) => {
    const n = (it.name || '').toLowerCase();
    if (it.gender && dropGenders.has(it.gender)) return false;
    if (exBrand && exBrand.test(it.brand || '')) return false;
  ...
  if (refine) {
    ...
    const RE_LONG = /длин\w*\s*рукав|рукавами|с рукавом|длинн\w*\s*рукав/i;
    ...
    for (let i = 0; i < scanN; i++) {
      ...
      const sleeveOpt = (card?.options?.['Длина рукава'] || card?.options?.['Вид рукава'] || '').toLowerCase();
      const txt = `${it.name || ''} ${card?.description || ''} ${sostav}`.toLowerCase();
      if (refine.longSleeve && (RE_SHORT.test(txt) || /коротк|без рукав/.test(sleeveOpt))) continue;
      // Положительный признак длинного рукава (в тексте или в характеристике).
      if (refine.requireLong && !(RE_LONG.test(txt) || /длин/.test(sleeveOpt))) continue;
      if (refine.jabot && !RE_JABOT.test(txt)) continue;
      if (inc && !inc.test(txt)) continue;
      if (exc && exc.test(txt)) continue;
      it._card = card; // для агрегации размеров из card.json
      kept.push(it);
    }
    if (kept.length) refinedPool = kept; // даже если меньше 10 — лучше чистые, чем с мусором
  }

  // Преобладающие размеры из card.json уточнённого пула (надёжно, без search.wb.ru).
  const sizeTally = new Map();
  for (const it of refinedPool.slice(0, top300)) {
    for (const v of (it._card?.sizesTable?.values || [])) {
      const sz = String(v.tech_size ?? v.techSize ?? '').trim().toUpperCase();
      if (sz) sizeTally.set(sz, (sizeTally.get(sz) || 0) + 1);
    }
  }
  ...
  // Преобладающие размеры: сначала из card.json уточнённого пула (надёжно), иначе — search.wb.ru.
  let dominantSizes = [];
  const SIZE_ORDER = ['XS', 'S', 'M', 'L', 'XL', 'XXL', '2XL', 'XXXL', '3XL', '4XL', '5XL', '6XL'];
  if (sizeTally.size) {
    const tot = [...sizeTally.values()].reduce((a, b) => a + b, 0) || 1;
    dominantSizes = [...sizeTally.entries()]
      .map(([size, offered]) => ({ size, offered, share: offered / tot }))
      .sort((a, b) => (SIZE_ORDER.indexOf(a.size) - SIZE_ORDER.indexOf(b.size)) || b.offered - a.offered);
  }
  if (!dominantSizes.length && sizesQuery) { try { dominantSizes = await fetchDominantSizes(sizesQuery, 3); } catch (_) { dominantSizes = []; } }
