...
/**
 * Имена переменных окружения, под которыми может лежать токен WB, по приоритету.
 * `Wildberries_API` — имя, под которым токен задан в облачной среде проекта.
 */
export const TOKEN_ENV_NAMES = ['WB_CONTENT_TOKEN', 'WB_TOKEN', 'Wildberries_API', 'WILDBERRIES_API'];

/** Возвращает имя переменной, в которой найден токен, либо null. Само значение не возвращает. */
export function tokenSource() {
  return TOKEN_ENV_NAMES.find((n) => (process.env[n] || '').trim().length > 0) || null;
}
