...
export function computeFoldedMonthlyProfile(active, opts = {}) {
  ...
  // Ячейки по (год-месяц): сумма/дни/лучший день.
  const cells = new Map();
  for (const r of active) {
    const ym = r.date.slice(0, 7);
    let c = cells.get(ym);
    if (!c) { c = { ym, month: Number(ym.slice(5)), year: Number(ym.slice(0, 4)), sales: 0, days: 0, bestDay: r.date, bestSales: -1 }; cells.set(ym, c); }
  ...
  for (const c of cells.values()) {
    ...
    if (!m) { m = { month: c.month, num: 0, den: 0, samples: 0, bestYear: -1, bestDay: c.bestDay, repDate: c.bestDay }; byMonth.set(c.month, m); }
    m.num += avgDaily * w; m.den += w; m.samples += 1;
    if (c.year >= m.bestYear) { m.bestYear = c.year; m.bestDay = c.bestDay; m.repDate = c.bestDay; } // репрезентативный день — из свежего года
  }

  const months = [...byMonth.values()]
    .map((m) => ({ month: m.month, avgDaily: m.den ? m.num / m.den : 0, samples: m.samples, repDate: m.repDate, bestDay: m.bestDay }))
    .sort((a, b) => a.month - b.month);
  const meanAvg = mean(months.map((m) => m.avgDaily)) || 1;
  for (const m of months) m.index = round(m.avgDaily / meanAvg, 3);
  return { months, asOf };
}
