...
export async function buildTopProducts({
  ...
  for (let i = 0; i < ranked.length; i++) {
    ...
    products.push({
      ...
      // доп. колонки: выкуп, возраст карточки, реклама, размерный ряд
      buyoutPct: it.buyoutPct,
      firstDate: it.firstDate,
      ageDays: daysSince(it.firstDate, d2),
      hasAd: it.hasAd,
      searchCpm: it.searchCpm,
      sizeCount: it.sizeCount,
...
// Возраст карточки: дней от firstDate до конца периода. null, если даты нет.
function daysSince(firstDate, d2) {
  if (!firstDate) return null;
  const m = String(firstDate).match(/\d{4}-\d{2}-\d{2}/);
  if (!m) return null;
  const a = new Date(m[0] + 'T00:00:00Z');
  const b = new Date(d2 + 'T00:00:00Z');
