...
> **Переезд на VPS уже начат:** сервер netcup настраивается по [`docs/vps-setup.md`](docs/vps-setup.md),
> скрипты — в [`deploy/vps/`](deploy/vps/README.md). Пока сервисы переезжают по одному, эта инструкция
> остаётся в силе для всего, что ещё живёт на Mac Mini. Funnel на Mac Mini по-прежнему не трогаем.
